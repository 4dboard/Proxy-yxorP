...
* Full name: `\MongoDB\Tests\UnifiedSpecTests\Constraint\IsBsonTypeTest`
...
```php
public testConstraint(mixed $type, mixed $value): void
```
...
```php
public testAny(mixed $type, mixed $value): void
```
...
### testAnyExcludesStream

```php
public testAnyExcludesStream(): void
```
...
```php
public testTypeJavascript(): void
```

***

### testTypeJavascriptWithScope

```php
public testTypeJavascriptWithScope(): void
```
...
```php
private assertResult(mixed $expected, \PHPUnit\Framework\Constraint\Constraint $constraint, mixed $value, string $message = &#039;&#039;): void
```
