...
* Full name: `\MongoDB\Tests\ClientFunctionalTest`
...
```php
private assertDatabaseExists(string $databaseName, callable $callback = null): void
```

An optional $callback may be provided, which should take a DatabaseInfo argument as its first and only parameter. If a
DatabaseInfo matching the given name is found, it will be passed to the callback, which may perform additional
assertions.
