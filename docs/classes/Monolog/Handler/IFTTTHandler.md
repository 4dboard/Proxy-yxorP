...
value1 will be the channel from monolog's Logger constructor, value2 will be the level name (ERROR, WARNING, ..)
value3 will be the log record's message

* Full name: `\Monolog\Handler\IFTTTHandler`
...
```php
public __construct(string $eventName, string $secretKey, int $level = Logger::ERROR, bool $bubble = true): mixed
```
...
|-----------|------|-------------|
| `$eventName` | **string** | The name of the IFTTT Maker event that should be triggered |
| `$secretKey` | **string** | A valid IFTTT secret key |
