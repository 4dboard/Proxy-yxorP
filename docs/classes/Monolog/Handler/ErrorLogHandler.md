...
* Full name: `\Monolog\Handler\ErrorLogHandler`
...
```php
public __construct(int $messageType = self::OPERATING_SYSTEM, int $level = Logger::DEBUG, bool $bubble = true, bool $expandNewlines = false): mixed
```
...
|-----------|------|-------------|
| `$messageType` | **int** | Says where the error should go. |
| `$level` | **int** | The minimum logging level at which this handler will be triggered |
| `$bubble` | **bool** | Whether the messages that are handled can bubble up the stack or not |
| `$expandNewlines` | **bool** | If set to true, newlines in the message will be expanded to be take multiple log entries |
...
```php
public static getAvailableTypes(): array
```
