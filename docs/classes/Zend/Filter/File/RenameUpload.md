...
Store already filtered values, so we can filter multiple times the same file without being block by move_uploaded_file
internal checks
...
|-----------|------|-------------|
| `$flag` | **
bool** | When true, this filter will use the $_FILES[&#039;name&#039;]<br />as the target filename.<br />Otherwise, it uses the default &#039;target&#039; rules. |
