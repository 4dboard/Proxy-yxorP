...
When there is a simple Print statement, like {{ article }}, and if the sandbox is enabled, we need to check that the __
toString()
method is allowed if 'article' is an object.

Not used anymore, to be deprecated in 2.x and removed in 3.0

* Full name: `\Twig\Node\SandboxedPrintNode`
* Parent class: [`\Twig\Node\PrintNode`](./PrintNode.md)
...
```php
protected removeNodeFilter(\Twig\Node\Node $node): \Twig\Node\Node
```
