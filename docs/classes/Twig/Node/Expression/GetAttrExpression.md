...
* Full name: `\Twig\Node\Expression\GetAttrExpression`
...
```php
public __construct(\Twig\Node\Expression\AbstractExpression $node, \Twig\Node\Expression\AbstractExpression $attribute, ?\Twig\Node\Expression\AbstractExpression $arguments, string $type, int $lineno): mixed
```
...
| `$arguments` | **?\Twig\Node\Expression\AbstractExpression** |  |
| `$type` | **string** |  |
| `$lineno` | **int** |  |
