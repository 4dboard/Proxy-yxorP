...
For instance, when there is a simple Print statement, like {{ article }}, and if the sandbox is enabled, we need to
check that the __toString()
method is allowed if 'article' is an object. The same goes for {{ article|upper }} or {{ random(article) }}

* Full name: `\Twig\Node\CheckToStringNode`
* Parent class: [`\Twig\Node\Expression\AbstractExpression`](./Expression/AbstractExpression.md)
...
```php
public __construct(\Twig\Node\Expression\AbstractExpression $expr): mixed
```
