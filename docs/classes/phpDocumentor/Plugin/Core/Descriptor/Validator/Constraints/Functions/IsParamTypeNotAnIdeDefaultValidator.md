...
Whenever this validator triggers then it means that someone forgot to replace the pseudo type generated by the IDE with
the real type for the given parameter.

* Full name: `\phpDocumentor\Plugin\Core\Descriptor\Validator\Constraints\Functions\IsParamTypeNotAnIdeDefaultValidator`
* Parent
...
```php
private isTypeAnIdeDefault(\phpDocumentor\Descriptor\Type\UnknownTypeDescriptor|\phpDocumentor\Descriptor\Interfaces\TypeInterface $type): bool
```
...
|-----------|------|-------------|
| `$type` | **
\phpDocumentor\Descriptor\Type\UnknownTypeDescriptor&#124;\phpDocumentor\Descriptor\Interfaces\TypeInterface** |  |
