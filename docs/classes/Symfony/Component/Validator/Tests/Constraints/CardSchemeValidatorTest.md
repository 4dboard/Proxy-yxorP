...
* Full name: `\Symfony\Component\Validator\Tests\Constraints\CardSchemeValidatorTest`
...
```php
public testValidNumbers(mixed $scheme, mixed $number): mixed
```
...
```php
public testInvalidNumbers(mixed $scheme, mixed $number, mixed $code): mixed
```
