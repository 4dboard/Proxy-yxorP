...
* Full name: `\Symfony\Component\Validator\Tests\Constraints\LocaleValidatorTest`
* Parent
...
```php
public testValidLocales(mixed $locale): mixed
```
...
```php
public testInvalidLocales(mixed $locale): mixed
```
...
```php
public getInvalidLocales(): mixed
```
