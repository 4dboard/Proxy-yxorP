...
* Full name: `\Symfony\Component\Validator\Tests\Constraints\LuhnValidatorTest`
* Parent
...
```php
public testValidNumbers(mixed $number): mixed
```
...
```php
public testInvalidNumbers(mixed $number, mixed $code): mixed
```
...
```php
public getInvalidNumbers(): mixed
```
...
```php
public testInvalidTypes(mixed $number): mixed
```
