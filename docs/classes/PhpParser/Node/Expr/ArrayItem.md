...
* Full name: `\PhpParser\Node\Expr\ArrayItem`
...
```php
public __construct(\PhpParser\Node\Expr $value, null|\PhpParser\Node\Expr $key = null, bool $byRef = false, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$value` | **\PhpParser\Node\Expr** | Value |
| `$key` | **null&#124;\PhpParser\Node\Expr** | Key |
