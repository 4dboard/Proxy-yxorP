...
* Full name: `\PhpParser\Node\Expr\BinaryOp`
...
```php
public __construct(\PhpParser\Node\Expr $left, \PhpParser\Node\Expr $right, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$left` | **\PhpParser\Node\Expr** | The left hand side expression |
| `$right` | **\PhpParser\Node\Expr** | The right hand side expression |
