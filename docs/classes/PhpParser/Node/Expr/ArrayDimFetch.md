...
* Full name: `\PhpParser\Node\Expr\ArrayDimFetch`
...
```php
public null|\PhpParser\Node\Expr $dim
```
...
```php
public __construct(\PhpParser\Node\Expr $var, null|\PhpParser\Node\Expr $dim = null, array $attributes = array()): mixed
```
...
| `$dim` | **null&#124;\PhpParser\Node\Expr** | Array index / dim |
