...
* Full name: `\PhpParser\Node\Expr\Array_`
...
```php
public \PhpParser\Node\Expr\ArrayItem[] $items
```
...
```php
public __construct(\PhpParser\Node\Expr\ArrayItem[] $items = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$items` | **\PhpParser\Node\Expr\ArrayItem[]** | Items of the array |
