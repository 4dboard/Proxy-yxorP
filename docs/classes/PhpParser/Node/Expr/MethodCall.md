...
* Full name: `\PhpParser\Node\Expr\MethodCall`
...
```php
public __construct(\PhpParser\Node\Expr $var, string|\PhpParser\Node\Expr $name, \PhpParser\Node\Arg[] $args = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$var` | **\PhpParser\Node\Expr** | Variable holding object |
