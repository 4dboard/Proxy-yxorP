...
* Full name: `\PhpParser\Node\Expr\List_`
...
```php
public \PhpParser\Node\Expr[] $vars
```
...
```php
public __construct(\PhpParser\Node\Expr[] $vars, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$vars` | **\PhpParser\Node\Expr[]** | List of variables to assign to |
