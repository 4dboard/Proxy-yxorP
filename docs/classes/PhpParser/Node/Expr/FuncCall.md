...
* Full name: `\PhpParser\Node\Expr\FuncCall`
...
```php
public \PhpParser\Node\Name|\PhpParser\Node\Expr $name
```
...
```php
public __construct(\PhpParser\Node\Name|\PhpParser\Node\Expr $name, \PhpParser\Node\Arg[] $args = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$name` | **\PhpParser\Node\Name&#124;\PhpParser\Node\Expr** | Function name |
