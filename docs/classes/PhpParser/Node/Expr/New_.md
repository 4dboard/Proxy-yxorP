...
* Full name: `\PhpParser\Node\Expr\New_`
...
```php
public \PhpParser\Node\Name|\PhpParser\Node\Expr|\PhpParser\Node\Stmt\Class_ $class
```
...
```php
public __construct(\PhpParser\Node\Name|\PhpParser\Node\Expr|\PhpParser\Node\Stmt\Class_ $class, \PhpParser\Node\Arg[] $args = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$class` | **\PhpParser\Node\Name&#124;\PhpParser\Node\Expr&#124;\PhpParser\Node\Stmt\Class_** | Class name (or class node for anonymous classes) |
