...
* Full name: `\PhpParser\Node\Stmt\Catch_`
...
```php
public \PhpParser\Node\Name $type
```
...
```php
public __construct(\PhpParser\Node\Name $type, string $var, \PhpParser\Node[] $stmts = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$type` | **\PhpParser\Node\Name** | Class of exception |
