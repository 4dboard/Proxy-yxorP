...
* Full name: `\PhpParser\Node\Stmt\Echo_`
...
```php
public \PhpParser\Node\Expr[] $exprs
```
...
```php
public __construct(\PhpParser\Node\Expr[] $exprs, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$exprs` | **\PhpParser\Node\Expr[]** | Expressions |
