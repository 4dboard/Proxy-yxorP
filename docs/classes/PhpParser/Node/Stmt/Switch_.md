...
* Full name: `\PhpParser\Node\Stmt\Switch_`
...
```php
public \PhpParser\Node\Stmt\Case_[] $cases
```
...
```php
public __construct(\PhpParser\Node\Expr $cond, \PhpParser\Node\Stmt\Case_[] $cases, array $attributes = array()): mixed
```
...
| `$cases` | **\PhpParser\Node\Stmt\Case_[]** | Case list |
