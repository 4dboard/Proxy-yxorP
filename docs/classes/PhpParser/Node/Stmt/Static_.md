...
* Full name: `\PhpParser\Node\Stmt\Static_`
...
```php
public \PhpParser\Node\Stmt\StaticVar[] $vars
```
...
```php
public __construct(\PhpParser\Node\Stmt\StaticVar[] $vars, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$vars` | **\PhpParser\Node\Stmt\StaticVar[]** | Variable definitions |
