...
* Full name: `\PhpParser\Node\Stmt\ClassConst`
...
```php
public \PhpParser\Node\Const_[] $consts
```
...
```php
public __construct(\PhpParser\Node\Const_[] $consts, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$consts` | **\PhpParser\Node\Const_[]** | Constant declarations |
