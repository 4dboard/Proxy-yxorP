...
* Full name: `\PhpParser\Node\Stmt\Declare_`
...
```php
public \PhpParser\Node\Stmt\DeclareDeclare[] $declares
```
...
```php
public __construct(\PhpParser\Node\Stmt\DeclareDeclare[] $declares, \PhpParser\Node[] $stmts, array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$declares` | **\PhpParser\Node\Stmt\DeclareDeclare[]** | List of declares |
