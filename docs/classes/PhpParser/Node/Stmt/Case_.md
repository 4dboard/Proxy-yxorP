...
* Full name: `\PhpParser\Node\Stmt\Case_`
...
```php
public null|\PhpParser\Node\Expr $cond
```
...
```php
public __construct(null|\PhpParser\Node\Expr $cond, \PhpParser\Node[] $stmts = array(), array $attributes = array()): mixed
```
...
|-----------|------|-------------|
| `$cond` | **null&#124;\PhpParser\Node\Expr** | Condition (null for default) |
