...
| `$options` | **
array** | Options array. The boolean &#039;throwOnError&#039; option determines whether an exception should be<br />thrown on first error, or if the parser should try to continue parsing the remaining code<br />and build a partial AST. |
...
Array of statements (or null if the 'throwOnError' option is disabled and the parser was unable to recover from an
error).
