...
Note that large files may be compresed differently than smaller files; see the "Large File Support" section above for
more information.
...
| `$path` | **
string** | - path to file on disk (note: paths should be encoded using<br />UNIX-style forward slashes -- e.g &#039;/path/to/some/file&#039;). |
| `$opt` | **
...
// add a list of files to the archive $files = array('foo.txt', 'bar.jpg'); foreach ($files as $path)
