<?php define('YXORP_START_TIME', microtime(true));
...
function yxorp($module = null)
{
    ...
    if (!$app) {
        ...
        $config = array_replace_recursive(['debug' => preg_match('/(localhost|::1|\.local)$/', @$_SERVER['SERVER_NAME']), 'app.name' => 'yxorP', 'base_url' => YXORP_BASE_URL, 'base_route' => YXORP_BASE_ROUTE, 'docs_root' => YXORP_DOCS_ROOT, 'session.name' => md5(YXORP_ENV_ROOT), 'session.init' => (YXORP_ADMIN && !YXORP_API_REQUEST) ? true : false, 'sec-key' => 'c3b40c4c-db44-s5h7-a814-b4931a15e5e1', 'i18n' => 'en', 'database' => ['server' => 'mongolite://' . (YXORP_STORAGE_FOLDER . '/data'), 'options' => ['db' => 'yxorpdb'], 'driverOptions' => []], 'memory' => ['server' => 'redislite://' . (YXORP_STORAGE_FOLDER . '/data/yxorp.memory.sqlite'), 'options' => []], 'paths' => ['#root' => YXORP_DIR, '#storage' => YXORP_STORAGE_FOLDER, '#pstorage' => YXORP_PUBLIC_STORAGE_FOLDER, '#data' => YXORP_STORAGE_FOLDER . '/data', '#cache' => YXORP_STORAGE_FOLDER . '/cache', '#tmp' => YXORP_STORAGE_FOLDER . '/tmp', '#thumbs' => YXORP_PUBLIC_STORAGE_FOLDER . '/thumbs', '#uploads' => YXORP_PUBLIC_STORAGE_FOLDER . '/uploads', '#modules' => YXORP_DIR . '/modules', '#addons' => YXORP_ENV_ROOT . '/addons', '#config' => YXORP_CONFIG_DIR, 'assets' => YXORP_DIR . '/assets', 'site' => YXORP_SITE_DIR], 'filestorage' => [],], is_array($customConfig) ? $customConfig : []);
        ...
        $app->service('filestorage', function () use ($config, $app) {
            $storages = array_replace_recursive(['root' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('#root:')], 'mount' => true, 'url' => $app->pathToUrl('#root:', true)], 'site' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('site:')], 'mount' => true, 'url' => $app->pathToUrl('site:', true)], 'tmp' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('#tmp:')], 'mount' => true, 'url' => $app->pathToUrl('#tmp:', true)], 'thumbs' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('#thumbs:')], 'mount' => true, 'url' => $app->pathToUrl('#thumbs:', true)], 'uploads' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('#uploads:')], 'mount' => true, 'url' => $app->pathToUrl('#uploads:', true)], 'assets' => ['adapter' => 'League\Flysystem\Adapter\Local', 'args' => [$app->path('#uploads:')], 'mount' => true, 'url' => $app->pathToUrl('#uploads:', true)]], $config['filestorage']);
        ...
        $modulesPaths = array_merge([YXORP_DIR . '/modules', YXORP_DIR . '/addons'], $config['loadmodules'] ?? []);
