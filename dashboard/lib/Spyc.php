<?php if (!function_exists('spyc_load')) {
...
if (!class_exists('Spyc')) {
    class Spyc
    {
        ...
        private function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null)
        {
            ...
            if (trim($value) != $value) $value = $this->_doLiteralBlock($value, $indent);
            if (is_bool($value)) {
        ...
        private function _doFolding($value, $indent)
        {
            if ($this->_dumpWordWrap !== 0 && is_string($value) && strlen($value) > $this->_dumpWordWrap) {
                ...
                if ($this->setting_dump_force_quotes && is_string($value) && $value !== self::REMPTY) $value = '"' . $value . '"';
        ...
        private function loadWithSource($Source)
        {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                ...
                if ($literalBlockStyle) $lineArray = $this->revertLiteralPlaceHolder($lineArray, $literalBlock);
                $this->addArray($lineArray, $this->indent);
                foreach ($this->delayedPath as $indent => $delayedPath) $this->path[$indent] = $delayedPath;
        ...
        {
            if (!empty($input) && strpos($input, "\n") === false && file_exists($input)) $input = file_get_contents($input);
        ...
        private function _parseLine($line)
        {
            ...
            if ($this->startsMappedValue($line)) return $this->returnMappedValue($line);
            if ($this->isArrayElement($line)) return $this->returnArrayElement($line);
            if ($this->isPlainArray($line)) return $this->returnPlainArray($line);
        ...
        private function _toType($value)
        {
            ...
            if ($is_quoted) {
                $value = str_replace('\n', "\n", $value);
                if ($first_character == "'") return strtr(substr($value, 1, -1), array('\'\'' => '\'', '\\\'' => '\''));
                return strtr(substr($value, 1, -1), array('\\"' => '"', '\\\'' => '\''));
            }
            if (strpos($value, ' #') !== false && !$is_quoted) $value = preg_replace('/\s+#(.+)$/', '', $value);
            ...
            if (is_numeric($value) && preg_match('/^(-|)[1-9]+[0-9]*$/', $value)) {
                $intvalue = (int)$value;
                if ($intvalue != PHP_INT_MAX && $intvalue != ~PHP_INT_MAX) $value = $intvalue;
                return $value;
            }
        ...
        {
            if (count($incoming_data) > 1) return $this->addArrayInline($incoming_data, $incoming_indent);
            ...
            if (is_string($key) && $key == '<<') {
                ...
                if (!is_array($_arr)) $_arr = array($key => $value); else $_arr[$key] = $value;
            } else {
                if (!is_array($_arr)) {
                    $_arr = array($value);
                    $key = 0;
        ...
        function revertLiteralPlaceHolder($lineArray, $literalBlock)
        {
            foreach ($lineArray as $k => $_) {
                if (is_array($_)) $lineArray[$k] = $this->revertLiteralPlaceHolder($_, $literalBlock); else if (substr($_, -1 * strlen($this->LiteralPlaceHolder)) == $this->LiteralPlaceHolder) $lineArray[$k] = rtrim($literalBlock, " \r\n");
            }
            return $lineArray;
        }
        ...
        private function isArrayElement($line)
        {
            ...
            if (strlen($line) > 3) if (substr($line, 0, 3) == '---') return false;
            return true;
        }
