...
<div>
    <ul class="uk-breadcrumb">
        ...
        <li class="uk-active" data-uk-dropdown>
            ...
            <a><i class="uk-icon-bars"></i> {{ htmlspecialchars(@$form['label'] ? $form['label']:$form['name'],
                ENT_QUOTES, 'UTF-8') }}</a>
            ...
            <div class="uk-dropdown">
                <ul class="uk-nav uk-nav-dropdown">
                    ...
                    <li class="uk-text-truncate"><a href="@route('/forms/export/'.$form['name'])"
                                                    download="{{ $form['name'] }}.form.json">@lang('Export entries')</a>
                    </li>
                </ul>
            </div>
...
<div riot-view>
    ...
    <div class="uk-margin uk-text-muted uk-text-center" if="{ready && entries.length}">
        ...
        <img class="uk-svg-adjust" src="@url($form['icon'] ? 'assets:app/media/icons/'.$form['icon']:'forms:icon.svg')"
    ...
    <div>
        ...
        <div class="uk-width-medium-1-3 uk-viewport-height-1-2 uk-container-center uk-text-center uk-flex uk-flex-middle"
            ...
            <div class="uk-animation-scale uk-width-1-1 uk-text-muted">
                ...
                <img class="uk-svg-adjust"
                     src="@url($form['icon'] ? 'assets:app/media/icons/'.$form['icon']:'forms:icon.svg')" width="50"
        ...
        <div class="uk-clearfix uk-margin-top uk-flex uk-flex-middle" show="{!loading && entries.length}">
            ...
            <div class="uk-flex-item-1 uk-flex uk-flex-middle uk-h3">
                <div class="uk-margin-small-right"><img
                            src="@url($form['icon'] ? 'assets:app/media/icons/'.$form['icon']:'forms:icon.svg')"
                            width="40" alt="icon"></div>
                <strong class="uk-margin-small-right">{{ htmlspecialchars(@$form['label'] ?
                    $form['label']:$form['name'], ENT_QUOTES, 'UTF-8') }}</strong>
                @lang('Entries')
            </div>

            <div class="uk-animation-fade" if="{ selected.length }">

                <a class="uk-button uk-button-large uk-button-danger uk-flex-inline uk-flex-middle"
        ...
        <table class="uk-table uk-table-tabbed uk-table-striped uk-margin-large-top" if="{ entries.length }">
            ...
            <tr each="{entry, idx in entries}">
                <td width="20"><label>
                        <input class="uk-checkbox" type="checkbox" data-check data-id="{ entry._id }">
                    </label></td>
                <td>
                ...
                <td>
                    <a class="uk-text-danger" onclick="{ parent.remove }" title="@lang('Delete')"><i
