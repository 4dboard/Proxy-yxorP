<?php $this->module('singletons')->extend(['createSingleton' => function ($name, $data = []) {
    if (!trim($name)) {
        return false;
    }
    ...
    $time = time();
    $singleton = array_replace_recursive(['name' => $name, 'label' => $name, '_id' => $name, 'fields' => [], 'data' => null, '_created' => $time, '_modified' => $time], $data);
    ...
    if ($singleton = $this->singleton($name)) {
        $options = array_merge(['user' => false, 'populate' => false, 'lang' => false, 'ignoreDefaultFallback' => false], $options);
        ...
        if ($options['populate'] && function_exists('yxorp_populate_collection')) {
            $fieldsFilter = [];
            if ($options['user']) $fieldsFilter['user'] = $options['user'];
            if ($options['lang']) $fieldsFilter['lang'] = $options['lang'];
            $_items = [$data];
            $_items = yxorp_populate_collection($_items, intval($options['populate']), 0, $fieldsFilter);
            $data = $_items[0];
        }
...
$this->module('singletons')->extend(['getSingletonsInGroup' => function ($group = null) {
    if (!$group) {
