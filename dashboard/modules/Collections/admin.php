<?php $app->on('admin.init', function () {
    ...
    $this->helper('admin')->addMenuItem('modules', ['label' => 'Collections', 'icon' => 'collections:icon.svg', 'route' => '/collections', 'active' => $active]);
    if ($active) {
    ...
    $this->on('yxorp.search', function ($search, $list) {
        foreach ($this->module('collections')->getCollectionsInGroup() as $collection => $meta) {
            if (stripos($collection, $search) !== false || stripos($meta['label'], $search) !== false) {
                $list[] = ['icon' => 'database', 'title' => $meta['label'] ? $meta['label'] : $meta['name'], 'url' => $this->routeUrl('/collections/entries/' . $meta['name'])];
            }
        }
    });
    $this->on("admin.dashboard.widgets", function ($widgets) {
        $collections = $this->module("collections")->getCollectionsInGroup(null, false);
        $widgets[] = ["name" => "collections", "content" => $this->view("collections:views/widgets/dashboard.php", compact('collections')), "area" => 'aside-left'];
    }, 100);
    $this->on('yxorp.webhook.events', function ($triggers) {
        foreach (['collections.createcollection', 'collections.find.after', 'collections.find.after.{$name}', 'collections.find.before', 'collections.find.before.{$name}', 'collections.remove.after', 'collections.remove.after.{$name}', 'collections.remove.before', 'collections.remove.before.{$name}', 'collections.removecollection', 'collections.removecollection.{$name}', 'collections.reorder', 'collections.reorder.{$name}', 'collections.save.after', 'collections.save.after.{$name}', 'collections.save.before', 'collections.save.before.{$name}', 'collections.updatecollection', 'collections.updatecollection.{$name}'] as &$evt) {
            $triggers[] = $evt;
        }
    });
