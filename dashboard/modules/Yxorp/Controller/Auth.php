...
class Auth extends \LimeExtra\Controller {
    ...
    public function check() {
        ...
        if ($auth = $this->param('auth')) {
            ...
            if (!$this->app->helper('csrf')->isValid('login', $this->param('csrf'), true)) {
                $this->app->trigger('yxorp.authentication.failed', [$auth, 'Csrf validation failed']);
                return ['success' => false, 'error' => 'Csrf validation failed'];
            }

            $user = $this->module('yxorp')->authenticate($auth);

            if ($user && !$this->module('yxorp')->hasaccess('yxorp', 'backend', $user['group'] ?? null)) {
                $user = null;
            }
            ...
            if ($user) {
                ...
                $this->module('yxorp')->setUser($user);

                unset($user['api_key'], $user['_reset_token']);

            } else {
                $this->app->trigger('yxorp.authentication.failed', [$auth, 'Authentication failed']);
            }
    ...
    public function requestreset() {
        ...
        if ($user = $this->param('user')) {
            ...
            $user = $this->app->storage->findOne('yxorp/accounts', $query);
    ...
    public function newpassword() {
        ...
        if ($token = $this->param('token')) {
            ...
            $user = $this->app->storage->findOne('yxorp/accounts', ['_reset_token' => $token]);
            ...
            return $this->render('yxorp:views/layouts/newpassword.php', compact('user', 'token'));
        }
