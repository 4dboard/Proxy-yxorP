<?php $this->helpers['revisions'] = 'yxorP\\Helper\\Revisions';
$this->helpers['updater'] = 'yxorP\\Helper\\Updater';
$this->helpers['async'] = 'yxorP\\Helper\\Async';
$this->helpers['jobs'] = 'yxorP\\Helper\\Jobs';
$this->module('yxorp')->extend(['markdown' => function ($content, $extra = false) use ($app) {
    static $parseDown;
    ...
    if (file_exists($container)) {
        ...
        $keys = $this->app->storage->getKey('yxorp', 'api_keys', $keys);
    }
    return $keys;
    ...
    $options = array_merge(['cachefolder' => 'thumbs://', 'src' => '', 'mode' => 'thumbnail', 'mime' => null, 'fp' => null, 'filters' => [], 'width' => false, 'height' => false, 'quality' => 100, 'rebuild' => false, 'base64' => false, 'output' => false, 'redirect' => false,], $options);
    extract($options);
    if (!$width && !$height) {
    ...
    if (strpos($src, $this->app->filestorage->getUrl('assets://')) === 0) {
        ...
        $asset = $this->app->storage->findOne('yxorp/assets', ['_id' => $src]);
    }
    if ($asset) {
        ...
        if ($src) {
            $src = str_replace(YXORP_SITE_DIR, '', $src);
        }
    ...
    if ($rebuild || !$this->app->filestorage->has($thumbpath)) {
        try {
            ...
            $_filters = ['blur', 'brighten', 'colorize', 'contrast', 'darken', 'desaturate', 'edgeDetect', 'emboss', 'flip', 'invert', 'opacity', 'pixelate', 'sepia', 'sharpen', 'sketch'];
            ...
            return "data:image/gif;base64,R0lGODlhAQABAJEAAAAAAP///////wAAACH5BAEHAAIALAAAAAABAAEAAAICVAEAOw==";
        }
    }
    if ($base64) {
...
if (YXORP_ADMIN_CP) {
    ...
    $this->bind('/yxorp-api.js', function () {
        ...
        return $this->view('yxorp:views/api.js', compact('token', 'apiurl'));
    });
}
if (YXORP_CLI) {
