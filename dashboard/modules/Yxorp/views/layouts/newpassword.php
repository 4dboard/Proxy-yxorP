<!doctype html>
<html lang="{{ $app('i18n')->locale }}" class="uk-height-1-1 app-page-newpassword" data-base="@base('/')"
...
<div class="uk-position-relative container uk-animation-scale" riot-view>
    ...
    <form class="uk-form" method="post" action="@route('/auth/check')" onsubmit="{ submit }">
        ...
        <div id="newpassword-dialog" class="newpassword-dialog uk-panel-box uk-panel-space uk-panel-card uk-nbfc"
             show="{!$user}">
            ...
            <div class="uk-text-center uk-animation-slide-bottom" if="{ reset }">
                <a class="uk-button uk-button-outline uk-button-large uk-button-primary uk-width-1-1"
                   href="{{ $app->retrieve('yxorp.login.url', $app->routeUrl('/auth/login')) }}">@lang('Back to
                    Login')</a>
            </div>

            <div class="uk-form-row" show="{ !reset }">
                <input ref="password" class="uk-form-large uk-width-1-1 uk-text-center" type="password"
                       placeholder="@lang('New Password')" autofocus required>
            </div>
