...
class logPluginAction extends wrapper
{
    ...
    public function onHeadersReceived(): void
    {
        ...
        if (is_writable(realpath('./storage'))) @file_put_contents(realpath('./storage') . '/' . date("Y-m-d") . '.log', implode("\t", array('ip' => constants::get(YXORP_SERVER)['REMOTE_ADDR'], 'time' => time(), VAR_URL => constants::get(YXORP_REQUEST)->getUri(), 'status' => constants::get(YXORP_RESPONSE)->getStatusCode(), 'type' => constants::get(YXORP_RESPONSE)->headers->get('content-type', 'unknown'), 'size' => constants::get(YXORP_RESPONSE)->headers->get('content-length', 'unknown'))) . "\r\n", FILE_APPEND);
    }
}