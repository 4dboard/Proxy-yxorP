...
class yP
{
    ...
    final public static function proxy(string $root, array|null $request = null): void
    {
        ...
        /* It's setting the `YXORP_ACTIONS` constant to an array of files in the `DIR_ROOT . DIR_INC . DIR_ACTION`
        directory. */
        if (!CACHED_CONTEXT) self::set('YXORP_ACTIONS', scandir(DIR_ROOT . DIR_INC . DIR_ACTION));

        /* It's setting the `$actions` variable to an array of files in the `DIR_ROOT . DIR_INC . DIR_ACTION` directory. */
        $actions = [DIR_INC . DIR_ACTION => self::get('YXORP_ACTIONS'), DIR_PLUGIN => self::get(YXORP_TARGET_PLUGINS) ?: []];

        /* It's looping through all the files in the `DIR_ROOT . DIR_INC . DIR_ACTION` directory, and if the file is a
        ...
        /* Setting the token GUZZLE to a new instance of the \yxorP\inc\proxy class. */
    ...
    public static function init(?array $request, string $root): void
    {
        ...
        foreach (['PATH_DIR_COCKPIT' => DIR_ROOT . DIR_COCKPIT . DIR_STORAGE, 'PATH_COCKPIT_LOCAL' => DIR_ROOT . DIR_INC . DIR_STORAGE, 'PATH_COCKPIT_INDEX' => DIR_ROOT . DIR_COCKPIT . FILE_INDEX, 'PATH_REWRITE' => DIR_ROOT . DIR_INC . DIR_DATA . FILE_REWRITE, 'PATH_COCKPIT_BOOTSTRAP' => DIR_ROOT . DIR_COCKPIT . FILE_COCKPIT_BOOTSTRAP, 'PATH_INC_WRAPPER' => DIR_ROOT . DIR_INC . FILE_WRAPPER, 'PATH_TLDS_ALPHA_BY_DOMAIN' => DIR_ROOT . DIR_INC . DIR_DATA . FILE_TLDS_ALPHA_BY_DOMAIN, 'PATH_PUBLIC_SUFFIX_LIST' => DIR_ROOT . DIR_INC . DIR_DATA . FILE_PUBLIC_SUFFIX_LIST, 'PATH_FILE_MIME_TYPES' => DIR_ROOT . DIR_INC . DIR_DATA . FILE_MIME_TYPES, 'PATH_GUZZLE' => DIR_ROOT . DIR_INC . FILE_GUZZLE, 'PATH_BUGSNAG' => DIR_ROOT . DIR_INC . FILE_BUGSNAG] as $key => $value) define($key, $value);

        /* Checking if the files exist in the directory. */
        foreach (array('http', 'minify', 'parser') as $_asset) self::autoLoader(DIR_ROOT . 'inc' . DIRECTORY_SEPARATOR . $_asset . DIRECTORY_SEPARATOR);        // Reporting

        /* Loading the global variables from the cache. */
        foreach ($cached = \yxorP\inc\http\cache::fetch(CACHE_KEY_CONTEXT) as $key => $value) if ($key !== YXORP_COCKPIT_APP) yP::override($key, $value);
