...
global $_errorHandler_already_setup, $_errorHandler_global_handler, $_errorHandler_is_ini_enabled;
if (empty($_errorHandler_already_setup)) {
    $_errorHandler_already_setup = true;
    $missingIdentifier = array('T_INSTEADOF', 'T_TRAIT', 'T_TRAIT_C', 'T_YIELD', 'T_FINALLY');
    $counter = 100001;
    foreach ($missingIdentifier as $id) if (!defined($id)) define($id, $counter++);
    if (!isset($_errorHandler_global_handler)) {
        $_errorHandler_global_handler = null;
        $_errorHandler_is_ini_enabled = false;
        $_errorHandler_is_ini_enabled = !@get_cfg_var('errorHandler.force_disabled') && !@get_cfg_var('errorHandler.force_disable') && @ini_get('display_errors') === '1' && PHP_SAPI !== 'cli';
    }
    function withoutErrors($callback)
    {
        global $_errorHandler_global_handler;
        if ($_errorHandler_global_handler !== null) return $_errorHandler_global_handler->withoutErrors($callback); else return $callback();
    }
    ...
    class errorHandler
    {
        ...
        const MAGIC_IS_PRETTY_ERRORS_MARKER = '<!-- __magic_errorHandler_is_a_stack_trace_constant__ -->';
        const HEADER_SAVE_FILE = 'PHP_ERROR_SAVE_FILES';
        const POST_FILE_LOCATION = 'errorHandler_upload_file';
        const PHP_ERROR_INI_PREFIX = 'errorHandler';
        ...
        private static function isBinaryRequest()
        {
            ...
            foreach ($response as $key => $value) if (strtolower($key) === 'content-transfer-encoding') return strtolower($value) === 'binary';
        }

        private static function isNonPHPRequest()
        {
            $response = errorHandler::getResponseHeaders();
            foreach ($response as $key => $value) if (strtolower($key) === 'content-type') {
                foreach (errorHandler::$ALLOWED_RETURN_MIME_TYPES as $type) if (stripos($value, $type) !== false) return false;
                return true;
            }
            return false;
        }
        ...
        {
            if (isset(errorHandler::$PHP_SYMBOL_MAPPINGS[$symbol])) return errorHandler::$PHP_SYMBOL_MAPPINGS[$symbol]; else return "'$symbol'";
        }
        ...
        private static function syntaxHighlight($code)
        {
            ...
            for ($i = 1; $i < $len; $i++) {
                ...
                } else if ($code === '&') if ($i < $len) {
                    $next = $tokens[$i + 1];
                    if (is_array($next) && $next[0] === T_VARIABLE) $type = 'reference_ampersand';
                } else if ($code === '"' || $code === "'") {
                    if ($inString) {
                        $html[] = "<span class='syntax-string'>" . join('', $stringBuff) . htmlspecialchars($code) . "</span>";
                        $stringBuff = null;
                        $skip = true;
                    } else $stringBuff = array();
                    $inString = !$inString;
                } else if ($type === T_STRING) {
                    $matches = array();
                    preg_match(errorHandler::REGEX_PHP_CONST_IDENTIFIER, $code, $matches);
                    if ($matches && strlen($matches[0]) === strlen($code)) $type = 'const';
                }
                if ($skip) $skip = false; else {
                    $code = htmlspecialchars($code);
                    if ($type !== null && isset($syntaxMap[$type])) {
                        $class = $syntaxMap[$type];
                        if ($type === T_CONSTANT_ENCAPSED_STRING && strpos($code, "\\n") !== false) $append = "<span class='$class'>" . join("</span>\n<span class='$class'>", explode("\\n", $code)) . "</span>"; else if (strrpos($code, "\\n") === strlen($code) - 1) $append = "<span class='$class'>" . substr($code, 0, strlen($code) - 1) . "</span>\\n"; else $append = "<span class='$class'>$code</span>";
                    } else if ($inString && $code !== '"') $append = "<span class='syntax-string'>$code</span>"; else $append = $code;
                    if ($inString) $stringBuff[] = $append; else $html[] = $append;
                }
            }
            if ($stringBuff !== null) {
        ...
        private static function newArgument($name, $type = false, $isPassedByReference = false, $isOptional = false, $optionalValue = null, $highlight = false)
        {
            if ($name instanceof ReflectionParameter) {
                ...
                if ($klass !== null) $klass = $klass->name;
                ...
                if (strpos($paramType, '[') !== false || strlen($paramType) === 0) $paramType = null;
        ...
        private static function syntaxHighlightFunctionMatch($match, &$stackTrace, $highlightArg = null, &$numHighlighted = 0)
        {
            ...
            if ($className !== null) $reflectFun = new ReflectionMethod($className, $functionName); else if ($functionName === '{closure}') return '<span class="syntax-variable">$closure</span>'; else $reflectFun = new ReflectionFunction($functionName);
            if ($reflectFun) {
                ...
                if ($params) {
                    ...
                    foreach ($params as $i => $param) {
                        ...
                        if (!$arg['has_default']) $min = $i;
                        $args[] = $arg;
                    }
                    if ($highlightArg !== null) {
                        for ($i = $highlightArg; $i <= $min; $i++) $args[$i]['is_highlighted'] = true;
                        $numHighlighted = $min - $highlightArg;
                    }
                    if ($className !== null) if ($stackTrace && isset($stackTrace[1]) && isset($stackTrace[1]['type'])) $type = htmlspecialchars($stackTrace[1]['type']); else $type = null;
        ...
        private static function syntaxHighlightFunction($class, $type, $fun, &$args = null)
        {
            ...
            if ($args) {
                ...
                foreach ($args as $i => $arg) {
                    if ($i > 0) $info[] = ', ';
                    if (is_string($arg)) $info[] = $arg; else {
                        $highlight = $arg['is_highlighted'];
                        $name = $arg['name'];
                        if ($highlight) $info[] = '<span class="syntax-higlight-variable">';
                        if ($name === '_') $info[] = '<span class="syntax-variable-not-important">';
                        if ($arg['has_type']) {
                            $info[] = "<span class='syntax-class'>";
                            $info[] = $arg['type'];
                            $info[] = '</span> ';
                        }
                        if ($arg['is_reference']) $info[] = '<span class="syntax-function">&amp;</span>';
                        $info[] = "<span class='syntax-variable'>\$$name</span>";
                        if ($arg['has_default']) $info[] = '=<span class="syntax-literal">' . $arg['default_val'] . '</span>';
                        if ($name === '_') $info[] = '</span>';
                        if ($highlight) $info[] = '</span>';
                    }
                }
                $info[] = ' )';
            } else {
        ...
        {
            if ($type === errorHandler::FILE_TYPE_ROOT) return 'file-root'; else if ($type === errorHandler::FILE_TYPE_IGNORE) return 'file-ignore'; else if ($type === errorHandler::FILE_TYPE_APPLICATION) return 'file-app'; else return 'file-common';
        }
        ...
        private static function isFolderType(&$folders, $longest, $file)
        {
            ...
            for ($i = $len; $i > 0; $i--) if (isset($folders[$i])) {
                $folderParts =& $folders[$i];
                $success = false;
                for ($j = 0; $j < count($folderParts); $j++) {
                    $folderNames = $folderParts[$j];
                    for ($k = 0; $k < count($folderNames); $k++) if ($folderNames[$k] === $parts[$k]) $success = true; else {
                        $success = false;
                        break;
                    }
                }
        ...
        private static function setFolders(&$origFolders, &$longest, $folders)
        {
            $newFolders = array();
            $newLongest = 0;
            if ($folders) if (is_array($folders)) foreach ($folders as $folder) errorHandler::setFoldersInner($newFolders, $newLongest, $folder); else if (is_string($folders)) errorHandler::setFoldersInner($newFolders, $newLongest, $folders); else throw new Exception("Unknown value given for folder: " . $folders);
        ...
        private static function setFoldersInner(&$newFolders, &$newLongest, $folder)
        {
            ...
            if (isset($newFolders[$count])) {
                ...
                $folds[] = $parts;
            } else $newFolders[$count] = array($parts);
        }

        private static function getRequestHeaders()
        {
            if (function_exists('getallheaders')) return getallheaders(); else {
                $headers = array();
        ...
        public static function identifyTypeHTML($arg, $recurseLevels = 1)
        {
            if (!is_array($arg) && !is_object($arg)) {
                if (is_string($arg)) return "<span class='syntax-string'>&quot;" . htmlentities($arg) . "&quot;</span>"; else return "<span class='syntax-literal'>" . var_export($arg, true) . '</span>';
            } else if (is_array($arg)) {
                if (count($arg) === 0) return "[]"; else if ($recurseLevels > 0) {
                    $argArr = array();
                    foreach ($arg as $ag) $argArr[] = errorHandler::identifyTypeHTML($ag, $recurseLevels - 1);
                    if (($recurseLevels % 2) === 0) return "[" . join(', ', $argArr) . "]"; else return "[ " . join(', ', $argArr) . " ]";
                } else return "[...]";
            } else if (get_class($arg) === 'Closure') return '<span class="syntax-variable">$Closure</span>()'; else {
                $argKlass = get_class($arg);
                if (preg_match(errorHandler::REGEX_PHP_CONST_IDENTIFIER, $argKlass)) return '<span class="syntax-literal">$' . $argKlass . '</span>'; else return '<span class="syntax-variable">$' . $argKlass . '</span>';
            }
        }
        ...
        {
            global $_errorHandler_global_handler;
            if ($_errorHandler_global_handler !== null) $this->lastGlobalerrorHandler = $_errorHandler_global_handler; else $this->lastGlobalerrorHandler = null;
            $_errorHandler_global_handler = $this;
            ...
            if ($ignoreFolders !== null) errorHandler::setFolders($this->ignoreFolders, $this->ignoreFoldersLongest, $ignoreFolders);
            if ($appFolders !== null) errorHandler::setFolders($this->applicationFolders, $this->applicationFoldersLongest, $appFolders);
            ...
            if (!is_string($dir)) throw new Exception("Document root not found: " . $this->applicationRoot); else $this->applicationRoot = str_replace('\\', '/', $dir);
            ...
            if ($wordpress) $this->defaultErrorReportingOn = E_ERROR | E_WARNING | E_PARSE | E_USER_DEPRECATED & ~E_DEPRECATED & ~E_STRICT;
            $concrete5 = errorHandler::optionsPop($options, 'concrete5', false);
            if ($concrete5) $this->defaultErrorReportingOn = E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED;
            if ($options) foreach ($options as $key => $val) throw new InvalidArgumentException("Unknown option given $key");
            $this->isAjax = (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest')) || (isset($_REQUEST['errorHandler_is_ajax']));
        ...
        public function turnOn()
        {
            ...
            global $_errorHandler_is_ini_enabled;
            if ($_errorHandler_is_ini_enabled) if ($this->isSavingEnabled) {
                $headers = errorHandler::getRequestHeaders();
                if (isset($headers[errorHandler::HEADER_SAVE_FILE])) if (isset($_POST) && isset($_POST[errorHandler::POST_FILE_LOCATION])) {
                    $files = $_POST[errorHandler::POST_FILE_LOCATION];
                    foreach ($files as $file => $content) @file_put_contents($file, stripcslashes($content));
                    exit(0);
                }
            }
            return $this;
        ...
        {
            if (!is_callable($callback)) throw new Exception("non callable callback given");
            if ($this->isOn()) {
                $this->turnOff();
                $result = $callback();
                $this->turnOn();
        ...
        {
            global $_errorHandler_is_ini_enabled;
            if ($_errorHandler_is_ini_enabled) {
                if ($this->isOn()) {
                    $error = error_get_last();
                    if ($error && ($error['type'] === 1 || $error['type'] === 4 || $error['type'] === 64)) $this->reportError($error['type'], $error['message'], $error['line'], $error['file']); else $this->endBuffer();
                } else $this->endBuffer();
            }
        }
        ...
        {
            global $_errorHandler_is_ini_enabled;
            if ($_errorHandler_is_ini_enabled && !$this->isBufferSetup) {
                $this->isBufferSetup = true;
                ini_set('implicit_flush', false);
                ob_implicit_flush(false);
                if (!@ini_get('output_buffering')) @ini_set('output_buffering', 'on');
                $output = '';
                $bufferOutput = true;
            ...
            if ($this->isBufferSetup) {
                ...
                for ($i = count($handlers) - 1; $i >= 0; $i--) {
                    ...
                    if ($handler === 'ob_gzhandler') {
                        $wasGZHandler = true;
                        ob_end_clean();
                    } else if ($handler === 'default output handler') ob_end_clean(); else ob_end_flush();
                }
                $content = $this->discardBuffer();
                if ($wasGZHandler) ob_start('ob_gzhandler'); else ob_start();
        ...
        private function getFolderType($root, $file)
        {
            ...
            if ($file === __FILE__) $type = errorHandler::FILE_TYPE_IGNORE; else if (strpos($testFile, '/') === false) $type = errorHandler::FILE_TYPE_ROOT; else if ($this->isApplicationFolder($testFile)) $type = errorHandler::FILE_TYPE_APPLICATION; else if ($this->isIgnoreFolder($testFile)) $type = errorHandler::FILE_TYPE_IGNORE; else $type = false;
            return array($type, $testFile);
        }

        private function getFileContents($path)
        {
            if (isset($this->cachedFiles[$path])) return $this->cachedFiles[$path]; else {
                ...
                if ($lines) {
                    ...
                    foreach ($fileLines as $i => $line) {
                        ...
                        if (strlen($newLine) > 0) {
                            ...
                            if ($stripSize === -1) $stripSize = $numSpaces; else $stripSize = min($stripSize, $numSpaces);
                        } else $fileLines[$i] = $newLine;
                    }
                    if ($stripSize > 0) {
                        if ($stripSize > 4) $stripSize -= 4;
                        foreach ($fileLines as $i => $line) if (strlen($line) > $stripSize) $fileLines[$i] = substr($line, $stripSize);
                    }
                    $fileLines = join("\\n", $fileLines);
                    $fileLines = errorHandler::syntaxHighlight($fileLines);
                    $fileLines = explode("\\n", $fileLines);
                    $lines = array();
                    for ($i = 0; $i < count($fileLines); $i++) $lines[$i + $minLine + 1] = $fileLines[$i];
                }
                return $lines;
            } catch (Exception $ex) {
                return null;
        ...
        private function removeRootPath($root, $path)
        {
            ...
            if (strpos($filePath, $root) === 0 && strlen($root) < strlen($filePath)) return substr($filePath, strlen($root) + 1); else return $filePath;
        }
        ...
        private function improveErrorMessage($ex, $code, $message, $errLine, $errFile, $root, &$stackTrace)
        {
            ...
            $skipStackFirst = function (&$stackTrace) {
                ...
                foreach ($stackTrace as $i => $trace) {
                    if ($skipFirst) $skipFirst = false; else {
                        if ($trace && isset($trace['file']) && isset($trace['line'])) return array($trace['file'], $trace['line'], $i);
                    }
                }
            ...
            if ($code === 1) {
                if ((strpos($message, " undefined method ") !== false) || (strpos($message, " undefined function ") !== false)) {
                    ...
                    if ($matches) {
                        ...
                        if ($stackTrace && isset($stackTrace[1]) && $stackTrace[1]['args']) {
                            ...
                            for ($i = 0; $i < $numArgs; $i++) $args[] = errorHandler::newArgument("_");
                        }
                ...
                } else if ($message === 'Using $this when not in object context') $message = 'Using <span class="syntax-variable">$this</span> outside object context'; else if (strpos($message, "Class ") !== false && strpos($message, "not found") !== false) {
                    $matches = array();
                ...
                if (strpos($message, "Missing argument ") === 0) {
                    ...
                    if ($matches) {
                        ...
                        if ($numHighlighted > 0) $message = preg_replace('/^Missing argument ([0-9]+)/', 'Missing arguments ', $message);
                        if ($altInfo) {
                ...
                } else if (strpos($message, 'require(') === 0 || strpos($message, 'include(') === 0) {
                    $endI = strpos($message, '):');
                    if ($endI) {
                        ...
                        if (strpos($message, $replaceBit) === $endI + 2) $message = substr_replace($message, 'no ', $endI + 2, strlen($replaceBit));
                ...
                if ($message === "syntax error, unexpected T_ENCAPSED_AND_WHITESPACE") $message = "syntax error, string is not closed"; else {
                    ...
                    if ($semiColonError) {
                        ...
                        if ($num > 0) {
                            ...
                            foreach ($symbols as $i => $sym) $symbols[$i] = errorHandler::phpSymbolToDescription($sym);
                            $newMatch = join(', or ', $symbols);
                            $message = str_replace($match, ", expecting $newMatch", $message);
                        }
                    }
                }
            } else if ($code === 8) if (strpos($message, "Undefined variable:") !== false) {
                $matches = array();
                preg_match(errorHandler::REGEX_VARIABLE, $message, $matches);
                if (count($matches) > 0) $message = 'Undefined variable <span class="syntax-variable">$' . $matches[0] . '</span>';
            } else if ($code === 4096) {
                if (strpos($message, 'must be an ')) {
                    ...
                    if ($matches) {
                        ...
                        if ($fun) {
                            ...
                            $scalarType = null;
                            if (!errorHandler::$IS_SCALAR_TYPE_HINTING_SUPPORTED) foreach (errorHandler::$SCALAR_TYPES as $scalar) if (stripos($message, "must be an instance of $scalar,") !== false) {
                                $scalarType = $scalar;
                                break;
                            }
                            if ($scalarType !== null) {
                                ...
                                } else $message = preg_replace('/^Argument /', 'Incorrect argument ', $message);
                            }
                            if ($prioritizeCaller) list($srcErrFile, $srcErrLine, $stackSearchI) = $skipStackFirst($stackTrace);
                        }
                    }
                }
            }
            if ($stackTrace !== null) {
                ...
                if ($isEmpty) array_unshift($stackTrace, array('line' => $errLine, 'file' => $errFile)); else if (count($stackTrace) > 0 && ((!isset($stackTrace[0]['line'])) || ($stackTrace[0]['line'] !== $errLine))) array_unshift($stackTrace, array('line' => $errLine, 'file' => $errFile));
        ...
        private function parseStackTrace($code, $message, $errLine, $errFile, &$stackTrace, $root, $altInfo = null)
        {
            if ($stackTrace !== null) {
                ...
                foreach ($stackTrace as $i => $trace) {
                    if ($trace) {
                        if (isset($trace['line'])) $lineLen = max($lineLen, strlen($trace['line'])); else $trace['line'] = '';
                        $info = '';
                        if ($i === 0 && $altInfo !== null) $info = $altInfo; else if ($i > 0 && (isset($trace['class']) || isset($trace['type']) || isset($trace['function']))) {
                            $args = array();
                            if (isset($trace['args'])) foreach ($trace['args'] as $arg) $args[] = errorHandler::identifyTypeHTML($arg, 1);
                        ...
                        } else if (isset($trace['info']) && $trace['info'] !== '') $info = errorHandler::syntaxHighlight($trace['info']); else if (isset($trace['file']) && !isset($trace['info'])) {
                            $contents = $this->getFileContents($trace['file']);
                            if ($contents) $info = errorHandler::syntaxHighlight(trim($contents[$trace['line'] - 1]));
                        }
                        $trace['info'] = $info;
                        if (isset($trace['file'])) {
                ...
                foreach ($stackTrace as $i => $trace) {
                    if ($trace) {
                        ...
                        $file = $trace['file'];
                        $fileKlass = '';
                        if ($trace['is_native']) $fileKlass = 'file-internal-php'; else $fileKlass = 'filename ' . errorHandler::folderTypeToCSS($trace['file_type']);
                        ...
                        if ($trace['is_native']) $cssClass = 'is-native '; else $cssClass = '';
                        if ($highlightI === $i) $cssClass .= ' highlight'; else if ($highlightI > $i) $cssClass .= ' pre-highlight';
                        ...
                        $data = '';
                        if (isset($trace['file-id'])) $data = ' data-file-id="' . $trace['file-id'] . '"' . ' data-line="' . $line . '"';
        ...
        private function logError($message, $file, $line, $ex = null)
        {
            if ($ex) {
                ...
                $parts = explode("\\n", $trace);
                $trace = "        " . join("\\n        ", $parts);
                if (!errorHandler::isIIS()) error_log("$message \n           $file, $line \n$trace");
            } else if (!errorHandler::isIIS()) error_log("$message \n           $file, $line");
        }
        ...
        {
            $this->reportError($ex->getCode(), $ex->getMessage(), $ex->getLine(), $ex->getFile(), $ex);
        }
        ...
        public function reportError($code, $message, $errLine, $errFile, $ex = null)
        {
            ...
            global $_errorHandler_is_ini_enabled;
            if ($_errorHandler_is_ini_enabled && $this->isOn() && ($this->isAjax || !$this->htmlOnly || !errorHandler::isNonPHPRequest())) {
        ...
        private function getStackTrace($ex, $code, $errFile, $errLine)
        {
            ...
            if ($ex !== null) {
                ...
                $stackTrace = array();
                $skipStacks = 0;
                for ($next = $ex; $next !== null; $next = $next->getPrevious()) {
                    $ex = $next;
                    $stack = $ex->getTrace();
                    $file = $ex->getFile();
                    $line = $ex->getLine();
                    if ($stackTrace !== null && count($stackTrace) > 0) $stack = array_slice($stack, 0, count($stack) - count($stackTrace) + 1);
                    if (count($stack) > 0 && (!isset($stack[0]['file']) || !isset($stack[0]['line']) || $stack[0]['file'] !== $file || $stack[0]['line'] !== $line)) array_unshift($stack, array('file' => $file, 'line' => $line));
                    $stackTrace = ($stackTrace !== null) ? array_merge($stack, $stackTrace) : $stack;
                    if (count($stackTrace) > 0) $stackTrace[0]['exception'] = $ex;
                }
                $message = $ex->getMessage();
                $errFile = $ex->getFile();
                $errLine = $ex->getLine();
                ...
                if (method_exists($ex, 'getSeverity')) {
                    ...
                    if ($code === 0 && $severity !== 0 && $severity !== null) $code = $severity;
                }
            }
        ...
        private function generateDumpHTML($arrays, $request, $response, $server)
        {
            $arrToHtml = function ($name, $array, $css = '') {
                $max = 0;
                foreach ($array as $e => $v) $max = max($max, strlen($e));
            ...
            $html = '';
            foreach ($arrays as $key => $value) if (isset($value) && $value) $html .= $arrToHtml($key, $value); else unset($arrays[$key]);
            return "<div class='error-dumps'>" . $html . $arrToHtml('request', $request, 'dump_request') . $arrToHtml('response', $response, 'dump_response') . $arrToHtml('server', $server, 'dump_server') . "</div>";
        }
        ...
        private function generateFileLineSets($srcErrFile, $srcErrLine, &$stackTrace)
        {
            ...
            if ($stackTrace) foreach ($stackTrace as $i => &$trace) if ($trace && isset($trace['file']) && isset($trace['line'])) {
                $file = $trace['file'];
                $line = $trace['line'];
                if (isset($seenFiles[$file])) $fileSet = $seenFiles[$file]; else {
        ...
        private function setEnabled($isOn)
        {
            ...
            $this->isOn = $isOn;
            global $_errorHandler_is_ini_enabled;
            if ($_errorHandler_is_ini_enabled) if (!$isOn) if ($wasOn) $this->runDisableErrors(); else if ($isOn) $this->runEnableErrors();
        }
        ...
        private function runDisableErrors()
        {
            ...
            if ($_errorHandler_is_ini_enabled) {
                ...
                if (errorHandler::isIIS()) @ini_restore('log_errors');
            }
        }
                ...
                if (!$self->isShutdownRegistered) {
                    if ($self->catchClassNotFound) {
                        ...
                        spl_autoload_register(function ($className) use ($self, &$classException, &$autoloaderFuns) {
                            if ($self->isOn()) {
                                $classException = null;
                                if (defined('DEBUG_BACKTRACE_IGNORE_ARGS')) $trace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS); else $trace = debug_backtrace();
                                $error = true;
                                foreach ($trace as $row) if (isset($row['function'])) {
                                    $function = $row['function'];
                                    if (in_array($function, $autoloaderFuns, true)) {
                                        $error = false;
                                        break;
                                    } else if ($function !== '__autoload' && $function !== 'spl_autoload_call' && strpos($function, 'errorHandler\\') === false) break;
                                }
                                if ($error) $classException = new ErrorException("Class '$className' not found", E_ERROR, 0, __FILE__, __LINE__);
                            }
                        });
                    }
        ...
        { ?>
            <script data-errorHandler="magic JS, just ignore this!">
                "use strict";
                    ...
                    if (window.XMLHttpRequest) {
                        ...
                        var copyRequestProperties = function (src, dest, includeReadOnly, skipResponse) {
                            ...
                            if (includeReadOnly) {
                                ...
                                if (!skipResponse) copyResponseProperties(src, dest);
                            }
                        ...
                        var runFail = function (ev) {
                            ...
                            iframe.onload = function () {
                                ...
                                if (retry) {
                                    ...
                                    iframe.__errorHandler_retry = retryFun;
                            ...
                            inner.onreadystatechange = function (ev) {
                                ...
                                if (!isAjaxError && self.onreadystatechange) {
                                    /*
                                         ...
                                         */
                                    if (state < 4) stateResults.push(copyRequestProperties(self, {}, true)); else {
        ...
        private function displayError($message, $errLine, $errFile, $errFileType, $stackTrace, &$fileLinesSets, $numFileLines, $dumpInfo)
        {
            ...
            if (isset($_SERVER['QUERY_STRING'])) {
                ...
                if ($requestUrlLen > 0 && substr($requestUrl, $requestUrlLen - 1) === '?') $requestUrl = substr($requestUrl, 0, $requestUrlLen - 1);
            } else $requestUrl = $_SERVER['REQUEST_URI'];
            header_remove('Content-Transfer-Encoding');
            $this->displayHTML(function () use ($message, $errFile, $errLine) {
                echo "<!--\\n" . "$message\n" . "$errFile, $errLine\n" . "-->";
            }, function () use ($requestUrl, $backgroundText, $serverName, $applicationRoot, $message, $errLine, $errFile, $errFileType, $stackTrace, &$fileLinesSets, $numFileLines, $displayLineNumber, $dumpInfo, $isSavingEnabled) {
                if ($backgroundText) { ?>
                    <div id="error-wrap">
                ...
                <div class="error-file-top <?php echo($fileLinesSets ? 'has_code' : '') ?>">
                    <h2 id="error-file"><span id="error-linenumber"><?php echo $errLine ?></span> <span
                                id="error-filename" class="<?php echo $errFileType ?>"><?php echo $errFile ?></span>
                    </h2>
                ...
                <?php if ($fileLinesSets) { ?>
                    ...
                    <?php foreach ($fileLinesSets as $i => $fileLinesSet) {
                ...
                if ($stackTrace !== null) echo $stackTrace;
                if ($dumpInfo !== null) echo $dumpInfo;
                    ...
                    $(document).ready(function () {
                        ...
                        if ($editor.size() > 0) {
                            ...
                            editor.on('change', function () {
                                if (currentFile !== null) changedFiles[currentFile] = true;
                            });

                            var selectFile = function (link, line) {
                                var fileID = link.attr('data-file-id');
                                if (line === undefined) line = link.attr('data-line');
                                ...
                                setTimeout(function () {
                                    ...
                                    for (var i = 0; i < files.size(); i++) {
                                        ...
                                        if (f.getAttribute('data-file-id') === fileID) file = f;
                                    }
                                    ...
                                    if (file) {
                                        ...
                                        if (fileSessions.hasOwnProperty(currentFile)) session = fileSessions[currentFile]; else {
                            ...
                            $('.error-stack-trace-line').mouseover(function () {
                                ...
                                if (!$this.hasClass('select-highlight')) $this.addClass('select-highlight');
                            ...
                            $('.error-file-save').click(function (ev) {
                                ...
                                if (window.top !== window) {
                                    ...
                                    for (var i = 0; i < arrFrames.length; i++) if (arrFrames[i].contentWindow === window) retryFun = arrFrames[i].__errorHandler_retry;
                                }
        ...
        function displayHTML(Closure $head, $body = null, $javascript = null)
        {
            ...
            @ob_clean();
            if (!$this->htmlOnly && errorHandler::isNonPHPRequest()) @header("Content-Type: text/html");
            ...
            <div class="background"><?php $body(); ?></div><?php ?>
            <script>
                (function () {
                    function h(a) {
                        var c = function (a, b) {
                    ...
                    var d = a("./lib/dom"), e = a("./lib/event"), f = a("./editor").Editor,
                        g = a("./edit_session").EditSession, h = a("./undomanager").UndoManager,
                        i = a("./virtual_renderer").VirtualRenderer, j = a("./multi_select").MultiSelect;
                    ...
                    var d = Function.prototype.call, e = Array.prototype, f = Object.prototype, g = e.slice,
                        h = d.bind(f.toString), i = d.bind(f.hasOwnProperty), j, k, l, m, n;
                    ...
                    a("./lib/fixoldbrowsers");
                    var d = a("./lib/oop"), e = a("./lib/lang"), f = a("./lib/useragent"),
                        g = a("./keyboard/textinput").TextInput, h = a("./mouse/mouse_handler").MouseHandler,
                        i = a("./mouse/fold_handler").FoldHandler, j = a("./keyboard/keybinding").KeyBinding,
                        k = a("./edit_session").EditSession, l = a("./search").Search, m = a("./range").Range,
                        n = a("./lib/event_emitter").EventEmitter, o = a("./commands/command_manager").CommandManager,
                        p = a("./commands/default_commands").commands, q = function (a, b) {
                    ...
                    (function () {
                        d.implement(this, n), this.setKeyboardHandler = function (a) {
                            ...
                            var c = b.start.column - 1, d = b.end.column + 1, e = a.getLine(b.start.row), f = e.length,
                            ...
                            this.clearSelection();
                            var g = d.column, h = b.getState(d.row), i = c.checkOutdent(h, b.getLine(d.row), a),
                                j = b.getLine(d.row),
                                k = c.getNextLineIndent(h, j.slice(0, d.column), b.getTabString()), l = b.insert(d, a);
                            ...
                            if (!(b.start.row < b.end.row || b.start.column < b.end.column)) {
                                ...
                                if (this.session.getUseSoftTabs()) {
                                    var f = a.getTabSize(), g = this.getCursorPosition(),
                                        h = a.documentToScreenColumn(g.row, g.column), i = f - h % f;
                                    d = e.stringRepeat(" ", i)
                                } else d = "	";
                            ...
                            if (c.isEmpty()) {
                                ...
                                var e = a.isBackwards(), f = a.isBackwards() ? c.start : c.end,
                            ...
                            var c = this.renderer, d = this.renderer.layerConfig,
                            ...
                            if (!c) {
                                c = this.find({
                                    needle: /[{}()\[\]]/g,
                                    preventScroll: !0,
                                    start: {row: b.row, column: b.column - 1}
                                });
                                if (!c) return;
                                var d = c.start;
                    ...
                    var d = a("../lib/event"), e = a("../lib/useragent"), f = a("./default_handlers").DefaultHandlers,
                        g = a("./default_gutter_handler").GutterHandler, h = a("./mouse_event").MouseEvent,
                        i = a("./dragdrop").DragdropHandler, j = function (a) {
                    ...
                    (function () {
                        this.onMouseDown = function (a) {
                            ...
                            var b, c = this.editor, d = c.renderer.screenToTextCoordinates(this.x, this.y),
                            ...
                            var b = h(this.mousedownEvent.x, this.mousedownEvent.y, this.x, this.y),
                                c = (new Date).getTime(), d = this.editor;
                    ...
                    var d = a("./config"), e = a("./lib/oop"), f = a("./lib/lang"), g = a("./lib/net"),
                        h = a("./lib/event_emitter").EventEmitter, i = a("./selection").Selection,
                        j = a("./mode/text").Mode, k = a("./range").Range, l = a("./document").Document,
                        m = a("./background_tokenizer").BackgroundTokenizer,
                        n = a("./search_highlight").SearchHighlight, o = function (a, b) {
                    ...
                    (function () {
                        ...
                        e.implement(this, h), this.setDocument = function (a) {
                            ...
                            if (this.$modified || a) {
                                ...
                                var b = this.doc.getAllLines(), c = this.$rowLengthCache, d = 0, e = 0,
                                    f = this.$foldData[e], g = f ? f.start.row : Infinity, h = b.length;
                                for (var i = 0; i < h; i++) {
                                    if (i > g) {
                                        i = f.end.row + 1;
                                        if (i >= h) break;
                            ...
                            var b = this.$useWrapMode, c, d = a.data.action, e = a.data.range.start.row,
                                f = a.data.range.end.row, g = a.data.range.start, h = a.data.range.end, i = null;
                            ...
                            var c = this.doc.getAllLines(), d = this.getTabSize(), e = this.$wrapData,
                                g = this.$wrapLimit, h, j, k = a;
                        ...
                        this.$computeWrapSplits = function (a, b) {
                            ...
                            var c, d = 0, e = 0, f, g = 0, h = 0, i = this.$screenRowCache,
                                j = this.$getRowCacheIndex(i, a);
                            if (0 < j && j < i.length) var g = i[j], d = this.$docRowCache[j],
                                k = a > g || a == g && j == i.length - 1; else var k = !0;
                            ...
                            if (0 < j && j < i.length) var h = i[j], d = this.$screenRowCache[j],
                                k = a > h || a == h && j == i.length - 1; else var k = !0;
                            ...
                            if (!this.$useWrapMode) {
                                ...
                                var e = this.$wrapData.length, f = 0, d = 0, b = this.$foldData[d++],
                    ...
                    var d = a("./lib/oop"), e = a("./lib/lang"), f = a("./lib/event_emitter").EventEmitter,
                        g = a("./range").Range, h = function (a) {
                    ...
                    (function () {
                        d.implement(this, f), this.isEmpty = function () {
                            ...
                            var a = this.lead.row, b = this.lead.column, c = this.session.documentToScreenRow(a, b),
                                d = this.session.screenToDocumentPosition(c, 0),
                                e = this.session.getDisplayLine(a, null, d.row, d.column), f = e.match(/^\s*/);
                            ...
                            var a = this.lead.row, b = this.lead.column, c = this.doc.getLine(a), d = c.substring(b),
                    ...
                    var d = a("../tokenizer").Tokenizer, e = a("./text_highlight_rules").TextHighlightRules,
                        f = a("./behaviour").Behaviour, g = a("../unicode"), h = function () {
                    ...
                    var d = a("./lib/oop"), e = a("./lib/event_emitter").EventEmitter, f = a("./range").Range,
                        g = a("./anchor").Anchor, h = function (a) {
                    ...
                    (function () {
                        d.implement(this, e), this.setValue = function (a) {
                            ...
                            var c = this.$split(b), d = c.splice(0, 1)[0],
                            ...
                            if (b == c) return;
                            var d = new f(a, b, a, c), e = this.getLine(a), g = e.substring(b, c),
                            ...
                            var c = new f(a, 0, b + 1, 0), d = this.$lines.splice(a, b - a + 1),
                                e = {action: "removeLines", range: c, nl: this.getNewLineCharacter(), lines: d};
                            return this._emit("change", {data: e}), d
                        }, this.removeNewLine = function (a) {
                            var b = this.getLine(a), c = this.getLine(a + 1), d = new f(a, b.length, a + 1, 0),
                                e = b + c;
                    ...
                    var d = a("./lib/oop"), e = a("./lib/event_emitter").EventEmitter,
                    ...
                    function h() {
                        this.getFoldAt = function (a, b, c) {
                            ...
                            var c = this.getFoldWidget(a), d = this.getLine(a), e = b.shiftKey,
                                f = e || b.ctrlKey || b.altKey || b.metaKey, g;
                    ...
                    var d = a("../range").Range, e = a("./fold_line").FoldLine, f = a("./fold").Fold,
                    ...
                    function f() {
                        this.findMatchingBracket = function (a) {
                            ...
                            var b = this.getLine(a.row), c = !0, d, f = b.charAt(a.column - 1),
                                g = f && f.match(/([\(\[\{])|([\)\]\}])/);
                    ...
                    (function () {
                        this.set = function (a) {
                            ...
                            var c = b.range, e = c ? a.getLines(c.start.row, c.end.row) : a.doc.getAllLines(), g = [],
                                h = b.re;
                            if (b.$isMultiLine) {
                                var i = h.length, j = e.length - i;
                                for (var k = h.offset || 0; k <= j; k++) {
                                    for (var l = 0; l < i; l++) if (e[k + l].search(h[l]) == -1) break;
                                    var m = e[k], n = e[k + i - 1], o = m.match(h[0])[0].length,
                    ...
                    var d = a("../lib/oop"), e = a("../keyboard/hash_handler").HashHandler,
                        f = a("../lib/event_emitter").EventEmitter, g = function (a, b) {
                    ...
                    var d = a("./lib/oop"), e = a("./lib/dom"), f = a("./lib/event"), g = a("./lib/useragent"),
                        h = a("./config"), i = a("./lib/net"), j = a("./layer/gutter").Gutter,
                        k = a("./layer/marker").Marker, l = a("./layer/text").Text, m = a("./layer/cursor").Cursor,
                        n = a("./scrollbar").ScrollBar, o = a("./renderloop").RenderLoop,
                        p = a("./lib/event_emitter").EventEmitter, q = a("ace/requirejs/text!./css/editor.css");
                    ...
                    (function () {
                        this.showGutter = !0, this.CHANGE_CURSOR = 1, this.CHANGE_MARKER = 2, this.CHANGE_GUTTER = 4, this.CHANGE_SCROLL = 8, this.CHANGE_LINES = 16, this.CHANGE_TEXT = 32, this.CHANGE_SIZE = 64, this.CHANGE_MARKER_BACK = 128, this.CHANGE_MARKER_FRONT = 256, this.CHANGE_FULL = 512, this.CHANGE_H_SCROLL = 1024, d.implement(this, p), this.setSession = function (a) {
                            ...
                            var a = this.session, b = this.scrollTop % this.lineHeight,
                                c = this.$size.scrollerHeight + this.lineHeight, d = this.$getLongestLine(),
                            ...
                            var h = Math.ceil(c / this.lineHeight) - 1,
                                i = Math.max(0, Math.round((this.scrollTop - b) / this.lineHeight)), j = i + h, k, l,
                                m = this.lineHeight;
                            ...
                            var c = this.scroller.getBoundingClientRect(),
                                ...
                                e = Math.floor((b + this.scrollTop - c.top) / this.lineHeight), f = Math.round(d);
                    ...
                    var d = a("../lib/dom"), e = a("../lib/oop"), f = a("../lib/event_emitter").EventEmitter,
                        g = function (a) {
                    ...
                    (function () {
                        e.implement(this, f), this.setSession = function (a) {
                            ...
                            var b = {className: ""}, c = [], e = a.firstRow, f = a.lastRow,
                                g = this.session.getNextFoldLine(e), h = g ? g.start.row : Infinity,
                                i = this.$showFoldWidgets && this.session.foldWidgets, j = this.session.$breakpoints,
                                k = this.session.$decorations, l = 0;
                            for (; ;) {
                            ...
                            var o = ("" + l).length * a.characterWidth, p = this.$padding || this.$computePadding();
                    ...
                    (function () {
                        this.$padding = 0, this.setPadding = function (a) {
                            ...
                            var f = e === "background" ? 0 : this.$padding, g = d.lineHeight,
                                h = this.$getTop(b.start.row, d), i = Math.round(f + b.start.column * d.characterWidth);
                    ...
                    var d = a("../lib/oop"), e = a("../lib/dom"), f = a("../lib/lang"), g = a("../lib/useragent"),
                        h = a("../lib/event_emitter").EventEmitter, i = function (a) {
                    ...
                    (function () {
                        d.implement(this, h), this.EOF_CHAR = "Â¶", this.EOL_CHAR = "Â¬", this.TAB_CHAR = "â†’", this.SPACE_CHAR = "Â·", this.$padding = 0, this.setPadding = function (a) {
                            ...
                            var d = Math.max(b, a.firstRow), f = Math.min(c, a.lastRow), g = this.element.childNodes,
                                h = 0;
                            ...
                            var d = this.element.ownerDocument.createDocumentFragment(), f = b,
                                g = this.session.getNextFoldLine(f), h = g ? g.start.row : Infinity;
                            for (; ;) {
                            ...
                            this.config = a;
                            var b = [], c = a.firstRow, d = a.lastRow, f = c, g = this.session.getNextFoldLine(f),
                    ...
                    var d = a("./lib/oop"), e = a("./lib/dom"), f = a("./lib/event"),
                        g = a("./lib/event_emitter").EventEmitter, h = function (a) {
                    ...
                    var d = a("./range_list").RangeList, e = a("./range").Range, f = a("./selection").Selection,
                        g = a("./mouse/multi_select_handler").onMouseDown, h = a("./lib/event"),
                    ...
                    (function () {
                        this.updateSelectionMarkers = function () {
                            ...
                            var c = this.selection.toOrientedRange(), d = c.cursor == c.end,
                            ...
                            if (!c.isEmpty()) var h = this.session.documentToScreenPosition(d ? c.end : c.start),
                                i = this.session.screenToDocumentPosition(h.row + a, h.column); else var i = g;
                            if (d) {
                    ...
                    function f(a) {
                        ...
                        var i = a.editor, j = i.selection, k = i.inMultiSelectMode, l = a.getDocumentPosition(),
                            m = j.getCursor(), n = a.inSelection() || j.isEmpty() && e(l, m), o = a.x, p = a.y,
                            q = function (a) {
                    ...
                    var d = a("../lib/oop"), e = a("../lib/event_emitter").EventEmitter, f = a("../config"),
                        g = function (b, c, d) {
                    ...
                    e.prototype = {
                        $buildKeymappingRegex: function (a) {
                            ...
                            if (b == 0 || c != "" && c != String.fromCharCode(0)) {
                                var g = this.$composeBuffer(a, b, c, f), h = g.bufferToUse, i = g.symbolicName,
                                    j = g.keyIdentifier;
                    ...
                    var d = a("./range").Range, e = a("./lib/event_emitter").EventEmitter, f = a("./lib/oop"),
                ...
                ace.define("ace/mode/php", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/php_highlight_rules", "ace/mode/matching_brace_outdent", "ace/range", "ace/mode/behaviour/cstyle", "ace/mode/folding/cstyle"], function (a, b, c) {
                    var d = a("../lib/oop"), e = a("./text").Mode, f = a("../tokenizer").Tokenizer,
                        g = a("./php_highlight_rules").PhpHighlightRules,
                        h = a("./matching_brace_outdent").MatchingBraceOutdent, i = a("../range").Range,
                        j = a("./behaviour/cstyle").CstyleBehaviour, k = a("./folding/cstyle").FoldMode,
                        l = function () {
                    ...
                    var d = a("../lib/oop"), e = a("../lib/lang"),
                        f = a("./doc_comment_highlight_rules").DocCommentHighlightRules,
                        g = a("./text_highlight_rules").TextHighlightRules, h = function () {
                            var a = f,
                                ...
                                d = e.arrayToMap("die|echo|empty|exit|eval|include|include_once|isset|list|require|require_once|return|print|unset".split("|")),
                                g = e.arrayToMap("true|false|null|__CLASS__|__DIR__|__FILE__|__LINE__|__METHOD__|__FUNCTION__|__NAMESPACE__".split("|")),
                                h = e.arrayToMap("$GLOBALS|$_SERVER|$_GET|$_POST|$_FILES|$_REQUEST|$_SESSION|$_ENV|$_COOKIE|$errorHandlermsg|$HTTP_RAW_POST_DATA|$http_response_header|$argc|$argv".split("|")),
                                ...
                                j = e.arrayToMap("cfunction|old_function".split("|")), k = e.arrayToMap([]);
                            this.$rules = {
                                start: [{token: "support.php_tag", regex: "<\\?(?:php|\\=)"}, {
                                    token: "support.php_tag",
                                    regex: "\\?>"
                                }, {token: "comment", regex: "<\\!--", next: "htmlcomment"}, {
                                    token: "meta.tag",
                                    regex: "<style",
                                    next: "css"
                                }, {token: "meta.tag", regex: "<\\/?[-_a-zA-Z0-9:]+", next: "htmltag"}, {
                                    token: "meta.tag",
                                    regex: "<!DOC" + "TYPE.*?>"
                                }, {token: "comment", regex: "\\/\\/.*$"}, {
                                    token: "comment",
                                ...
                                htmlcomment: [{token: "comment", regex: ".*?-->", next: "start"}, {
                                    token: "comment",
                                    regex: ".+"
                                }],
                                htmltag: [{token: "meta.tag", regex: ">", next: "start"}, {
                                    token: "text",
                                    regex: "[-_a-zA-Z0-9:]+"
                                }, {token: "text", regex: "\\s+"}, {token: "string", regex: '".*?"'}, {
                                    token: "string",
                                    regex: "'.*?'"
                                }],
                                css: [{token: "meta.tag", regex: "</style>", next: "htmltag"}, {
                                    token: "meta.tag",
                                    regex: ">"
                                }, {token: "text", regex: "(?:media|type|href)"}, {
                                    token: "string",
                                    regex: '=".*?"'
                                }, {token: "paren.lparen", regex: "{", next: "cssdeclaration"}, {
                                    token: "keyword",
                                    regex: "#[A-Za-z0-9-_.]+"
                                ...
                                cssdeclaration: [{
                                    ...
                                    regex: "[-a-zA-Z]+",
                                    next: "cssvalue"
                                }, {token: "paren.rparen", regex: "}", next: "css"}],
                                cssvalue: [{token: "text", regex: ":"}, {
                                    token: "constant",
                                    regex: "#[0-9a-zA-Z]+"
                                }, {token: "text", regex: "[-_0-9a-zA-Z\"' ,%]+"}, {
                                    token: "text",
                                    regex: ";",
                                    next: "cssdeclaration"
                                }]
                    ...
                    var d = a("../lib/oop"), e = a("./text_highlight_rules").TextHighlightRules, f = function () {
                        this.$rules = {
                            start: [{token: "comment.doc.tag", regex: "@[\\w\\d_]+"}, {
                                token: "comment.doc",
                                merge: !0,
                                regex: "\\s+"
                    ...
                    var d = a("../../lib/oop"), e = a("../behaviour").Behaviour, f = function () {
                        this.add("braces", "insertion", function (a, b, c, d, e) {
                            ...
                            if (e == "}") {
                                var h = c.getCursorPosition(), i = d.doc.getLine(h.row),
                            ...
                            if (e == '"' || e == "'") {
                                ...
                                var i = c.getCursorPosition(), j = d.doc.getLine(i.row),
                                    k = j.substring(i.column - 1, i.column);
                                if (k == "\\") return null;
                                var l = d.getTokens(g.start.row), m = 0, n, o = -1;
                    ...
                    var d = a("../../lib/oop"), e = a("../../range").Range, f = a("./fold_mode").FoldMode,
                    ...
                    (function () {
                        this.foldingStartMarker = null, this.foldingStopMarker = null, this.getFoldWidget = function (a, b, c) {
                            ...
                            if (g == -1) return;
                            var h = c || f.length, i = a.getLength(), j = b, k = b;
                            while (++b < i) {
                            ...
                            var g = {row: c, column: e + 1}, h = a.$findClosingBracket(b, g, f);
                            if (!h) return;
                ...
                (function (a, b) {
                    ...
                    var c = a.document, d = a.navigator, e = a.location, f = function () {
                        ...
                        var e = function (a, b) {
                            ...
                            }, f = a.jQuery, g = a.$, h, i = /^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/, j = /\S/,
                            k = /^\s+/, l = /\s+$/, m = /^<(\w+)\s*\/?>(?:<\/\1>)?$/, n = /^[\],:{}\s]*$/,
                            o = /\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,
                            p = /"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
                            q = /(?:^|:|,)(?:\s*\[)+/g, r = /(webkit)[ \/]([\w.]+)/,
                            s = /(opera)(?:.*version)?[ \/]([\w.]+)/, t = /(msie) ([\w.]+)/,
                            u = /(mozilla)(?:.*? rv:([\w.]+))?/, v = /-([a-z]|[0-9])/ig, w = /^-ms-/,
                            x = function (a, b) {
                                return (b + "").toUpperCase()
                            }, y = d.userAgent, z, A, B, C = Object.prototype.toString,
                            D = Object.prototype.hasOwnProperty, E = Array.prototype.push, F = Array.prototype.slice,
                            G = String.prototype.trim, H = Array.prototype.indexOf, I = {};
                        e.fn = e.prototype = {
                        ...
                        data: function (a, c, d, e) {
                            if (!!f.acceptData(a)) {
                                var g, h, i, j = f.expando, k = typeof c == "string", l = a.nodeType,
                                    m = l ? f.cache : a, n = l ? a[j] : a[j] && j, o = c === "events";
                    ...
                    var z = /^(?:textarea|input|select)$/i, A = /^([^\.]*)?(?:\.(.+))?$/, B = /(?:^|\s)hover(\.\S+)?\b/,
                        C = /^key/, D = /^(?:mouse|contextmenu)|click/, E = /^(?:focusinfocus|focusoutblur)$/,
                        F = /^(\w*)(?:#([\w\-]+))?(?:\.([\w\-]+))?$/, G = function (
                            a) {
                            var b = F.exec(a);
                    ...
                    f.event = {
                        ...
                        dispatch: function (c) {
                            ...
                            var d = (f._data(this, "events") || {})[c.type] || [], e = d.delegateCount,
                                g = [].slice.call(arguments, 0), h = !c.exclusive && !c.namespace,
                                i = f.event.special[c.type] || {}, j = [], k, l, m, n, o, p, q, r, s, t, u;
                        ...
                        var a = /((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g,
                            d = "sizcache" + (Math.random() + "").replace(".", ""), e = 0,
                    ...
                    var V = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
                        ...
                        bb = /<(?:script|object|embed|option|style)/i, bc = new RegExp("<(?:" + V + ")[\\s/>]", "i"),
                        bd = /checked\s*(?:[^=]|=\s*.checked.)/i, be = /\/(java|ecma)script/i,
                        bf = /^\s*<!(?:\[CDATA\[|\-\-)/, bg = {
                        ...
                        clone: function (a, b, c) {
                            ...
                            for (var k = 0, l; (l = a[k]) != null; k++) {
                                ...
                                if (typeof l == "string") if (!_.test(l)) l = b.createTextNode(l); else {
                                    l = l.replace(Y, "<$1></$2>");
                                    var m = (Z.exec(l) || ["", ""])[1].toLowerCase(), n = bg[m] || bg._default,
                                        o = n[0], p = b.createElement("div"), q = bh.childNodes, r;
                    ...
                    var bp = /alpha\([^)]*\)/i, bq = /opacity=([^)]*)/, br = /([A-Z]|^ms)/g,
                        bs = /^[\-+]?(?:\d*\.)?\d+$/i, bt = /^-?(?:\d*\.)?\d+(?!px)[^\d\s]+$/i,
                        bu = /^([\-+])=([\-+.\de]+)/, bv = /^margin/,
                    ...
                    var bC = /%20/g, bD = /\[\]$/, bE = /\r?\n/g, bF = /#.*$/, bG = /^(.*?):[ \t]*([^\r\n]*)\r?$/mg,
                        ...
                        bI = /^(?:about|app|app\-storage|.+\-extension|file|res|widget):$/, bJ = /^(?:GET|HEAD)$/,
                        bK = /^\/\//, bL = /\?/, bM = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
                        bN = /^(?:select|textarea)/i, bO = /\s+/, bP = /([?&])_=[^&]*/,
                        bQ = /^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+))?)?/, bR = f.fn.load, bS = {}, bT = {}, bU, bV,
                        bW = ["*/"] + ["*"];
                    try {
                        bU = e.href
                    } catch (bX) {
                    ...
                    bV = bQ.exec(bU.toLowerCase()) || [], f.fn.extend({
                        ...
                        ajax: function (a, c) {
                            ...
                            var d = f.ajaxSetup({}, c), e = d.context || d,
                                ...
                                i = f.Callbacks("once memory"), j = d.statusCode || {}, k, l = {}, m = {}, n, o, p, q,
                                r, s = 0, t, u, v = {
                    ...
                    "getBoundingClientRect" in c.documentElement ? cv = function (a, b, c, d) {
                        ...
                        var g = b.body, h = cy(b), i = c.clientTop || g.clientTop || 0,
    ...
    class JSMin
    {
                        ...
                        for (; ;) {
                            ...
                            if ($this->a === '[') {
                                for (; ;) {
                                    $this->output .= $this->a;
                                    $this->a = $this->get();
                                    if ($this->a === ']') break; elseif ($this->a === '\\') {
                                        $this->output .= $this->a;
                                        $this->a = $this->get();
                                    } elseif (ord($this->a) <= self::ORD_LF) throw new JSMinException('Unterminated regular expression set in regex literal.');
                                }
                            } elseif ($this->a === '/') break;
                            elseif ($this->a === '\\') {
                                $this->output .= $this->a;
                                $this->a = $this->get();
                            } elseif (ord($this->a) <= self::ORD_LF) throw new JSMinException('Unterminated regular expression literal.');
                            $this->output .= $this->a;
                        }
                        $this->b = $this->next();
                    }
        ...
        protected function get()
        {
            $c = $this->lookAhead;
            $this->lookAhead = null;
            if ($c === null) if ($this->inputIndex < $this->inputLength) {
        ...
        protected function min()
        {
            ...
            while ($this->a !== null) {
                switch ($this->a) {
                    case ' ':
                        if ($this->isAlphaNum($this->b)) $this->action(self::ACTION_KEEP_A); else $this->action(self::ACTION_DELETE_A);
                        break;
                    case "\\n":
                        switch ($this->b) {
                            case '{':
                            ...
                            case "\\n":
                                switch ($this->a) {
                                    ...
                                    case "'":
                                        ...
                                        break;
                                    default:
                                        if ($this->isAlphaNum($this->a)) $this->action(self::ACTION_KEEP_A); else $this->action(self::ACTION_DELETE_A_B);
                                }
                                break;
                            default:
                                $this->action(self::ACTION_KEEP_A);
    ...
    if ($_errorHandler_is_ini_enabled && $_errorHandler_global_handler === null && @get_cfg_var('errorHandler.autorun')) {
        reportErrors();
    }
}