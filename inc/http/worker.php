...
use yxorP\inc\connection\udpConnection;
use yxorP\inc\events\event;
use yxorP\inc\events\select;
...
class worker
{
    ...
    protected static function forkOneWorkerForLinux(self $worker)
    {
        ...
        if ($pid > 0) {
            ...
            $worker->id = $id;
            $worker->run();
            if (str_contains(static::$eventLoopClass, 'yxorP\inc\events\swoole')) exit(0);
    ...
    public function stop()
    {
        ...
        $this->unlisten();
        if (!static::$_gracefulStop) foreach ($this->connections as $connection) $connection->close();
