<?php namespace yxorP\inc\protocols;
...
class websocket implements protocolInterface
{
    ...
    public static function encode($buffer, connectionInterface $connection): string
    {
        ...
        if ($len <= 125) $encode_buffer = $first_byte . chr($len) . $buffer; else
            if ($len <= 65535) $encode_buffer = $first_byte . chr(126) . pack("n", $len) . $buffer; else
        ...
        if (empty($connection->websocketHandshake)) {
            ...
            if ($connection->maxSendBufferSize <= strlen($connection->tmpWebsocketData)) if ($connection->onBufferFull) try {
