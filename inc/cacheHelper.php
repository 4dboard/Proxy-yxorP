...
class cacheHelper
{
    public static function check($yxorp_root)
    {
        try {
            ...
            $array_map = array_map('str_getcsv', file($yxorp_root . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'mime.types'));
            /* Reading the mime types from the file `./data/mime.types` and storing it in the array `$mimeTypes`. */
            ...
            include $yxorp_root . DIRECTORY_SEPARATOR . 'tmp' . rtrim(strtr(base64_encode($_SERVER['SITE_URL'] . $_SERVER['REQUEST_URI']), '+/=', '._-')) . '.tmp';
