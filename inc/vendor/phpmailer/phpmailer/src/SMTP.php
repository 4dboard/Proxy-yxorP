...
class SMTP
{
     ...
     * @see SMTP::$do_debug
     */
    protected function edebug(string $str, int $level = 0)
    {
     ...
     */
    protected function getSMTPConnection(string $host, int $port = null, int $timeout = 30, array $options = []): bool
    {
        static $streamok;
     ...
     */
    protected function setError(string $message, string $detail = '', string $smtp_code = '', string $smtp_code_ex = '')
    {
        $this->error = [
            'error' => $message,
            'detail' => $detail,
     ...
     */
    protected function sendCommand(string $command, string $commandstring, array|int $expect): bool
    {
        if (!$this->connected()) {
     ...
     */
    public function client_send(string $data, string $command = ''): bool|int
    {
     ...
     * @param string|null $authtype The auth type (CRAM-MD5, PLAIN, LOGIN, XOAUTH2)
     ...
     */
    protected function parseHelloFields(string $type)
    {
        $this->server_caps = [];
        $lines = explode("\n", $this->helo_rply);
     ...
     */
    public function getServerExt(string $name): bool|string|null
    {
        if (!$this->server_caps) {
     ...
     * @param bool $enabled
     */
    public function setVerp(bool $enabled = false)
    {
     ...
     * @return string
     */
    public function getDebugOutput(): callable|string|LoggerInterface
    {
     ...
     */
    public function setDebugOutput(callable|string $method = 'echo')
    {
     ...
     * @param int $level
     */
    public function setDebugLevel(int $level = 0)
    {
        $this->do_debug = $level;
    }
     ...
     */
    public function getLastTransactionID(): bool|string|null
    {
        return $this->last_smtp_transaction_id;
    }
     ...
     */
    protected function errorHandler(int $errno, string $errmsg, string $errfile = '', int $errline = 0)
    {
