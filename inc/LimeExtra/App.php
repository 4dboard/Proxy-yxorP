<?php namespace LimeExtra;
class App extends \Lime\App
{
    public function __construct($settings = [])
    {
        $settings["helpers"] = \array_merge(['acl' => 'LimeExtra\\Helper\\SimpleAcl', 'assets' => 'LimeExtra\\Helper\\Assets', 'fs' => 'LimeExtra\\Helper\\Filesystem', 'image' => 'LimeExtra\\Helper\\Image', 'i18n' => 'LimeExtra\\Helper\\I18n', 'utils' => 'LimeExtra\\Helper\\Utils', 'cookie' => 'LimeExtra\\Helper\\Cookie', 'yaml' => 'LimeExtra\\Helper\\YAML',], $settings['helpers'] ?? []);
        parent::__construct($settings);
        $this->service('renderer', function () {
            $renderer = new \Lexy();
            $renderer->extend(function ($content) {
                $replace = ['extend' => '<?php $extend(expr); ?>', 'base' => '<?php $app->base(expr); ?>', 'route' => '<?php $app->route(expr); ?>', 'trigger' => '<?php $app->trigger(expr); ?>', 'assets' => '<?php echo $app->assets(expr); ?>', 'start' => '<?php $app->start(expr); ?>', 'end' => '<?php $app->end(expr); ?>', 'block' => '<?php $app->block(expr); ?>', 'url' => '<?php echo $app->pathToUrl(expr); ?>', 'view' => '<?php echo $app->view(expr); ?>', 'render' => '<?php echo $app->view(expr); ?>', 'include' => '<?php include($app->path(expr)); ?>', 'lang' => '<?php echo $app("i18n")->get(expr); ?>', 'json' => '<?php echo json_encode(expr); ?>',];
