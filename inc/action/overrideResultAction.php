...
class overrideResultAction extends wrapper
{
    ...
    public function onEventWrite(): void
    {
        ...
        if (constants::get(VAR_MIME) !== VAR_TEXT_HTML && constants::get(VAR_MIME) !== 'application/javascript' && constants::get(VAR_MIME) !== 'text/css' && constants::get(VAR_MIME) !== 'application/xml' && !str_contains(constants::get(VAR_MIME), VAR_TEXT) && !str_contains(constants::get(VAR_MIME), VAR_HTML)) return;
        ...
        constants::get(VAR_RESPONSE)->setContent((minify::createDefault())->process(constants::get(VAR_MIME) !== VAR_TEXT_HTML ? constants::get(VAR_RESPONSE)->getContent() : preg_replace_callback("(<(p|span|div|li|ul)(.*)>(.*)</(p|span|div|li|ul)>)", static function ($m) {
