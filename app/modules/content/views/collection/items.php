    ...
    <vue-view>
        ...
        <template>
            ...
            <div class="kiss-flex kiss-flex-middle kiss-margin-bottom">
                <div class="kiss-margin-small-right">
                    <kiss-svg class="kiss-margin-auto"
                              src="<?= $this->base(isset($model['icon']) && $model['icon'] ? $model['icon'] : 'content:assets/icons/collection.svg') ?>"
                ...
                <kiss-card class="kiss-overlay-input kiss-padding-small kiss-margin-small-right"
                           theme="contrast shadowed" v-if="Object.keys(App._locales).length > 1">
            ...
            <div
                ...
                v-if="fieldTypes && !loading && !items.length">
                <div>
                    <kiss-svg class="kiss-margin-auto" src="<?= $this->base('content:assets/icons/collection.svg') ?>"
                              width="40" height="40">
                ...
                <table class="kiss-table animated fadeIn" v-if="!loading && items.length">
                    ...
                    <tr>
                        ...
                        <th fixed="right" width="20">
                            <a class="kiss-size-4"
                               :class="model.fields.length != visibleFields.length ? 'kiss-color-danger': 'kiss-link-muted'"
                               kiss-popoutmenu="#model-column-options">
                                <icon>more_horiz</icon>
                            </a>
                        </th>
                    </tr>
                    ...
                    <tr v-for="item in items">
                        ...
                        <td class="kiss-align-center">
                            <icon :class="{'kiss-color-success': item._state === 1, 'kiss-color-danger': !item._state}">
                                trip_origin
                            </icon>
                        </td>
                        <td v-for="field in visibleFields">
                            <span class="kiss-badge kiss-badge-outline kiss-color-muted"
                                  v-if="item[field.name] == null">n/a</span>
                            <div class="kiss-text-truncate"
                                 v-else-if="fieldTypes[field.type] && fieldTypes[field.type].render"
                                 v-html="fieldTypes[field.type].render(item[field.name], field, 'table-cell')"></div>
                            <div class="kiss-text-truncate" v-else>
                                ...
                                <span class="kiss-badge kiss-badge-outline"
                                      v-else-if="typeof(item[field.name]) == 'object'">Object</span>
                                <span v-else>{{ item[field.name] }}</span>
                            </div>
                        </td>
                        <td><span class="kiss-flex kiss-badge kiss-badge-outline kiss-color-primary"
                                  :title="(new Date(item._modified * 1000).toLocaleString())">{{ (new Date(item._modified * 1000).toLocaleString()) }}</span>
                        </td>
                        <td><span class="kiss-flex kiss-badge kiss-badge-outline kiss-color-muted"
                                  :title="(new Date(item._created * 1000).toLocaleString())">{{ (new Date(item._created * 1000).toLocaleString()) }}</span>
                        </td>
            ...
            <teleport to="body">
                            ...
                            <ul>
                                ...
                                <li>
                                    <a class="kiss-flex kiss-flex-middle"
                                       :href="$route(`/content/collection/item/${model.name}/${actionItem._id}`)">
                ...
                <kiss-popoutmenu id="model-column-options" modal="true">
                    <kiss-content>
                        ...
                        <button type="button" class="kiss-button kiss-button-small kiss-width-1-1 kiss-margin-small-top"
                                kiss-popoutmenu-close><?= t('Close') ?></button>
                    </kiss-content>
                </kiss-popoutmenu>
                ...
                <kiss-container>
                    <div class="kiss-flex kiss-flex-middle">
                        ...
                        <div class="kiss-margin-right" v-if="selected.length">
                            <button class="kiss-button kiss-button-danger" @click="removeSelected()">{{ t('Delete') }}
                                -{{ selected.length }}-
                            </button>
                        </div>
                        <div class="kiss-button-group">
                            ...
                            <?php if ($this->helper('acl')->isAllowed("content/{$model['name']}/create")) : ?>
                                <a class="kiss-button kiss-button-primary"
                                   href="<?= $this->route("/content/collection/item/{$model['name']}") ?>"><?= t('Create item') ?></a>
                            <?php endif ?>
                        </div>
                    </div>
                </kiss-container>
