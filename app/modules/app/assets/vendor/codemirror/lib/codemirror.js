...
(function (global, factory) {
    ...
    const chrome = !edge && /Chrome\/(\d+)/.exec(userAgent);
    const chrome_version = chrome && +chrome[1];
    ...
    const sel_dontScroll = {scroll: false}, sel_mouse = {origin: "*mouse"}, sel_move = {origin: "+move"};
    ...
    function findFirst(pred, from, to) {
        ...
        for (; ;) {
            ...
            const midF = (from + to) / 2, mid = dir < 0 ? Math.ceil(midF) : Math.floor(midF);
            if (mid === from) {
    ...
    const bidiOrdering = (function () {
        ...
        const bidiRE = /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/;
        const isNeutral = /[stwN]/, isStrong = /[LRr]/, countsAsLeft = /[Lb1n]/, countsAsNum = /[1n]/;
    ...
    function off(emitter, type, f) {
        if (emitter.removeEventListener) {
            ...
            const map = emitter._handlers, arr = map && map[type];
            if (arr) {
    ...
    function getLine(doc, n) {
        ...
        while (!chunk.lines) {
            for (let i = 0; ; ++i) {
                const child = chunk.children[i], sz = child.chunkSize();
                if (n < sz) {
                    chunk = child;
                    break
                }
    ...
    function lineAtHeight(chunk, h) {
        ...
        outer: do {
            for (let i$1 = 0; i$1 < chunk.children.length; ++i$1) {
                const child = chunk.children[i$1], ch = child.height;
                if (h < ch) {
                    chunk = child;
                    continue outer
                }
        ...
        for (; i < chunk.lines.length; ++i) {
            const line = chunk.lines[i], lh = line.height;
            if (h < lh) {
                break
            }
            h -= lh;
    ...
    function highlightLine(cm, line, context, forceToEnd) {
        ...
        const st = [cm.state.modeGen], lineClasses = {};
    ...
    function takeToken(cm, pos, precise, asArray) {
        const doc = cm.doc, mode = doc.mode;
        let style;
        pos = clipPos(doc, pos);
        const line = getLine(doc, pos.line), context = getContextBefore(cm, pos.line, precise);
    ...
    function findStartLine(cm, n, precise) {
        ...
        for (let search = n; search > lim; --search) {
            ...
            const line = getLine(doc, search - 1), after = line.stateAfter;
    ...
    function markedSpansBefore(old, startCh, isInsert) {
        ...
        if (old) {
            for (let i = 0; i < old.length; ++i) {
                const span = old[i], marker = span.marker;
    ...
    function stretchSpansOverChange(doc, change) {
        ...
        const startCh = change.from.ch, endCh = change.to.ch, isInsert = cmp(change.from, change.to) === 0;
        // Get the spans that 'stick out' on both sides
        let first = markedSpansBefore(oldFirst, startCh, isInsert);
        let last = markedSpansAfter(oldLast, endCh, isInsert);

        // Next, merge those two ends
        const sameLine = change.text.length === 1, offset = lst(change.text).length + (sameLine ? startCh : 0);
        if (first) {
    ...
    function removeReadOnlyRanges(doc, from, to) {
        ...
        for (var i = 0; i < markers.length; ++i) {
            const mk = markers[i], m = mk.find(0);
            for (let j = 0; j < parts.length; ++j) {
                const p = parts[j];
                if (cmp(p.to, m.from) < 0 || cmp(p.from, m.to) > 0) {
                    continue
                }
                const newParts = [j, 1], dfrom = cmp(p.from, m.from), dto = cmp(p.to, m.to);
    ...
    function visualLineNo(doc, lineN) {
        const line = getLine(doc, lineN), vis = visualLine(line);
        if (line === vis) {
            return lineN
        }
        return lineNo(vis)
    ...
    function lineIsHiddenInner(doc, line, span) {
        ...
        let sp = (void 0), i = 0;
        for (; i < line.markedSpans.length; ++i) {
    ...
    const styleToClassCache = {}, styleToClassCacheWithMode = {};
    ...
    function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
        ...
        if (!special.test(text)) {
            ...
            while (true) {
                ...
                let txt$1 = (void 0);
                if (m[0] === "\t") {
                    const tabSize = builder.cm.options.tabSize, tabWidth = tabSize - builder.col % tabSize;
    ...
    function insertLineContent(line, builder, styles) {
        const spans = line.markedSpans, allText = line.text;
        let at = 0;
        if (!spans) {
        ...
        if (cm.options.lineNumbers || markers) {
            ...
            if (markers) {
                for (let k = 0; k < cm.display.gutterSpecs.length; ++k) {
                    const id = cm.display.gutterSpecs[k].className, found = markers.hasOwnProperty(id) && markers[id];
                    if (found) {
    ...
    function updateLineWidgets(cm, lineView, dims) {
        ...
        let node = lineView.node.firstChild, next = (void 0);
        for (; node; node = next) {
    ...
    function measureCharInner(cm, prepared, ch, bias) {
        ...
        let start = place.start, end = place.end, collapse = place.collapse;
        ...
        const rtop = rect.top - prepared.rect.top, rbot = rect.bottom - prepared.rect.top;
        ...
        const top = i ? heights[i - 1] : 0, bot = heights[i];
        const result = {
    ...
    function cursorCoords(cm, pos, context, lineObj, preparedMeasure, varHeight) {
        ...
        function getBidi(ch, partPos, invert) {
            const part = order[partPos], right = part.level === 1;
    ...
    function wrappedLineExtent(cm, lineObj, preparedMeasure, y) {
        ...
        const begin = findFirst(function (ch) {
    ...
    function coordsBidiPart(cm, lineObj, lineNo, preparedMeasure, order, x, y) {
        ...
        const index = findFirst(function (i) {
            const part = order[i], ltr = part.level !== 1;
    ...
    function getDimensions(cm) {
        const d = cm.display, left = {}, width = {};
        const gutterLeft = d.gutters.clientLeft;
        let n = d.gutters.firstChild, i = 0;
    ...
    function estimateHeight(cm) {
        const th = textHeight(cm.display), wrapping = cm.options.lineWrapping;
    ...
    function estimateLineHeights(cm) {
        const doc = cm.doc, est = estimateHeight(cm);
        doc.iter(function (line) {
    ...
    function regLineChange(cm, line, type) {
        cm.curOp.viewChanged = true;
        const display = cm.display, ext = cm.display.externalMeasured;
    ...
    function viewCuttingPoint(cm, oldN, newN, dir) {
        let index = findViewIndex(cm, oldN), diff;
        const view = cm.display.view;
    ...
    function adjustView(cm, from, to) {
        const display = cm.display, view = display.view;
    ...
    function prepareSelection(cm, primary) {
        ...
        const customCursor = cm.options.$customCursor;
        if (customCursor) {
            primary = true;
        }
        for (let i = 0; i < doc.sel.ranges.length; i++) {
            ...
            if (customCursor) {
                const head = customCursor(cm, range);
                if (head) {
    ...
    function drawSelectionCursor(cm, head, output) {
        ...
        if (/\bcm-fat-cursor\b/.test(cm.getWrapperElement().className)) {
            const charPos = charCoords(cm, head, "div", null, null);
            const width = charPos.right - charPos.left;
    ...
    function drawSelectionRange(cm, range, output) {
        const display = cm.display, doc = cm.doc;
        const fragment = document.createDocumentFragment();
        const padding = paddingH(cm.display), leftSide = padding.left;
        ...
        function drawForLine(line, fromArg, toArg) {
            ...
            iterateBidiSections(order, fromArg || 0, toArg === null ? lineLen : toArg, function (from, to, dir, i) {
                ...
                const openStart = fromArg === null && from === 0, openEnd = toArg === null && to === lineLen;
                const first = i === 0, last = !order || i === order.length - 1;
        ...
        const sFrom = range.from(), sTo = range.to();
        if (sFrom.line === sTo.line) {
            drawForLine(sFrom.line, sFrom.ch, sTo.ch);
        } else {
            const fromLine = getLine(doc, sFrom.line), toLine = getLine(doc, sTo.line);
    ...
    function updateHeightsInViewport(cm) {
        ...
        const viewTop = Math.max(0, display.scroller.getBoundingClientRect().top);
        let oldHeight = display.lineDiv.getBoundingClientRect().top;
        let mustScroll = 0;
        for (let i = 0; i < display.view.length; i++) {
            const cur = display.view[i], wrapping = cm.options.lineWrapping;
            let height = (void 0), width = 0;
            if (cur.hidden) {
                continue
            }
    ...
    function updateWidgetHeight(line) {
        if (line.widgets) {
            for (let i = 0; i < line.widgets.length; ++i) {
                const w = line.widgets[i], parent = w.node.parentNode;
                if (parent) {
    ...
    function visibleLines(display, doc, viewport) {
        ...
        if (viewport && viewport.ensure) {
            const ensureFrom = viewport.ensure.from.line, ensureTo = viewport.ensure.to.line;
            if (ensureFrom < from) {
                from = ensureFrom;
    ...
    function maybeScrollWindow(cm, rect) {
        ...
        const display = cm.display, box = display.sizer.getBoundingClientRect();
        let doScroll = null;
    ...
    function scrollPosIntoView(cm, pos, end, margin) {
        ...
        for (let limit = 0; limit < 5; limit++) {
            ...
            const startTop = cm.doc.scrollTop, startLeft = cm.doc.scrollLeft;
    ...
    function calculateScrollPos(cm, rect) {
        const display = cm.display, snapMargin = textHeight(cm.display);
        if (rect.top < 0) {
            rect.top = 0;
        }
        const screentop = cm.curOp && cm.curOp.scrollTop != null ? cm.curOp.scrollTop : display.scroller.scrollTop;
        const screen = displayHeight(cm), result = {};
        if (rect.bottom - rect.top > screen) {
            rect.bottom = rect.top + screen;
        }
        const docBottom = cm.doc.height + paddingVert(display);
        const atTop = rect.top < snapMargin, atBottom = rect.bottom > docBottom - snapMargin;
    ...
    function resolveScrollToPos(cm) {
        ...
        if (range) {
            ...
            const from = estimateCoords(cm, range.from), to = estimateCoords(cm, range.to);
    ...
    function measureForScrollbars(cm) {
        const d = cm.display, gutterW = d.gutters.offsetWidth;
    ...
    NativeScrollbars.prototype.enableZeroWidthBar = function (bar, delay, type) {
        ...
        function maybeDisable() {
            ...
            const elt = type === "vert" ? document.elementFromPoint(box.right - 1, (box.top + box.bottom) / 2)
    ...
    function endOperation_R1(op) {
        const cm = op.cm, display = cm.display;
    ...
    function endOperation_finish(op) {
        const cm = op.cm, display = cm.display, doc = cm.doc;
        ...
        // undoing
        const hidden = op.maybeHiddenMarkers, unhidden = op.maybeUnhiddenMarkers;
        if (hidden) {
    ...
    function highlightWorker(cm) {
        ...
        doc.iter(context.line, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function (line) {
            if (context.line >= cm.display.viewFrom) { // Visible
                ...
                const oldCls = line.styleClasses, newCls = highlighted.classes;
                if (newCls) {
    ...
    function patchDisplay(cm, updateNumbersFrom, dims) {
        const display = cm.display, lineNumbers = cm.options.lineNumbers;
    ...
    function alignHorizontally(cm) {
        ...
        const gutterW = display.gutters.offsetWidth, left = comp + "px";
        for (let i = 0; i < view.length; i++) {
            if (!view[i].hidden) {
    ...
    function maybeUpdateLineNumberWidth(cm) {
        ...
        const doc = cm.doc, last = lineNumberFor(cm.options, doc.first + doc.size - 1), display = cm.display;
        if (last.length !== display.lineNumChars) {
            const test = display.measure.appendChild(elt("div", [elt("div", last)],
                "CodeMirror-linenumber CodeMirror-gutter-elt"));
            const innerW = test.firstChild.offsetWidth, padding = test.offsetWidth - innerW;
    ...
    function renderGutters(display) {
        const gutters = display.gutters, specs = display.gutterSpecs;
    ...
    function onScrollWheel(cm, e) {
        ...
        const display = cm.display, scroll = display.scroller;
    ...
    Selection.prototype.equals = function (other) {
        ...
        for (let i = 0; i < this.ranges.length; i++) {
            const here = this.ranges[i], there = other.ranges[i];
    ...
    function normalizeSelection(cm, ranges, primIndex) {
        ...
        for (let i = 1; i < ranges.length; i++) {
            const cur = ranges[i], prev = ranges[i - 1];
            const diff = cmp(prev.to(), cur.from());
            if (mayTouch && !cur.empty() ? diff > 0 : diff >= 0) {
                const from = minPos(prev.from(), cur.from()), to = maxPos(prev.to(), cur.to());
    ...
    function computeReplacedSel(doc, changes, hint) {
        ...
        for (let i = 0; i < changes.length; i++) {
            ...
            newPrev = to;
            if (hint === "around") {
                const range = doc.sel.ranges[i], inv = cmp(range.head, range.anchor) < 0;
    ...
    function updateDoc(doc, change, markedSpans, estimateHeight) {
        ...
        const firstLine = getLine(doc, from.line), lastLine = getLine(doc, to.line);
        const lastText = lst(text), lastSpans = spansFor(text.length - 1), nlines = to.line - from.line;
    ...
    function addSelectionToHistory(doc, sel, opId, options) {
        const hist = doc.history, origin = options && options.origin;
    ...
    function mergeOldSpans(doc, change) {
        ...
        for (let i = 0; i < old.length; ++i) {
            const oldCur = old[i], stretchCur = stretched[i];
    ...
    function copyHistoryArray(events, newGroup, instantiateSel) {
        ...
        for (let i = 0; i < events.length; ++i) {
            ...
            const changes = event.changes, newChanges = [];
            copy.push({changes: newChanges});
            for (let j = 0; j < changes.length; ++j) {
                const change = changes[j], m = (void 0);
    ...
    function setSelectionReplaceHistory(doc, sel, options) {
        const done = doc.history.done, last = lst(done);
        if (last && last.ranges) {
    ...
    function skipAtomicInner(doc, pos, oldPos, dir, mayClear) {
        ...
        if (line.markedSpans) {
            for (let i = 0; i < line.markedSpans.length; ++i) {
                const sp = line.markedSpans[i], m = sp.marker;
                ...
                if ((sp.from === null || (preventCursorLeft ? sp.from <= pos.ch : sp.from < pos.ch)) &&
                    ...
                    if (oldPos) {
                        let near = m.find(dir < 0 ? 1 : -1), diff = (void 0);
    ...
    function makeChangeFromHistory(doc, type, allowSelectionOnly) {
        ...
        const hist = doc.history;
        let event, selAfter = doc.sel;
        const source = type === "undo" ? hist.done : hist.undone, dest = type === "undo" ? hist.undone : hist.done;
    ...
    function makeChangeSingleDocInEditor(cm, change, spans) {
        const doc = cm.doc, display = cm.display, from = change.from, to = change.to;
        ...
        const changesHandler = hasHandler(cm, "changes"), changeHandler = hasHandler(cm, "change");
    ...
    function rebaseHist(hist, change) {
        const from = change.from.line, to = change.to.line, diff = change.text.length - (to - from) - 1;
    ...
    BranchChunk.prototype = {
        ...
        removeInner: function (at, n) {
            ...
            for (let i = 0; i < this.children.length; ++i) {
                const child = this.children[i], sz = child.chunkSize();
                if (at < sz) {
                    const rm = Math.min(n, sz - at), oldHeight = child.height;
    ...
    LineWidget.prototype.clear = function () {
        const cm = this.doc.cm, ws = this.line.widgets, line = this.line, no = lineNo(line);
        if (no === null || !ws) {
            return
        }
    ...
    LineWidget.prototype.changed = function () {
        ...
        const oldH = this.height, cm = this.doc.cm, line = this.line;
        this.height = null;
    ...
    TextMarker.prototype.clear = function () {
        ...
        const cm = this.doc.cm, withOp = cm && !cm.curOp;
        if (withOp) {
            startOperation(cm);
        }
        ...
        if (cm && this.collapsed && !cm.options.lineWrapping) {
            for (let i$1 = 0; i$1 < this.lines.length; ++i$1) {
                const visual = visualLine(this.lines[i$1]), len = lineLength(visual);
    ...
    TextMarker.prototype.changed = function () {
        ...
        const pos = this.find(-1, true), widget = this, cm = this.doc.cm;
        if (!pos || !cm) {
            return
        }
        runInOp(cm, function () {
            const line = pos.line, lineN = lineNo(pos.line);
    ...
    function markText(doc, from, to, options, type) {
        ...
        const marker = new TextMarker(doc, type), diff = cmp(from, to);
        if (options) {
    ...
    function copySharedMarkers(doc, markers) {
        for (let i = 0; i < markers.length; i++) {
            const marker = markers[i], pos = marker.find();
            const mFrom = doc.clipPos(pos.from), mTo = doc.clipPos(pos.to);
            if (cmp(mFrom, mTo)) {
    ...
    function detachSharedMarkers(markers) {
        const loop = function (i) {
            const marker = markers[i], linked = [marker.primary.doc];
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {
        ...
        setValue: docMethodOp(function (code) {
            const top = Pos(this.first, 0), last = this.first + this.size - 1;
            makeChange(this, {
        ...
        getSelections: function (lineSep) {
            const parts = [], ranges = this.sel.ranges;
            for (let i = 0; i < ranges.length; i++) {
        ...
        findMarksAt: function (pos) {
            pos = clipPos(this, pos);
            const markers = [], spans = getLine(this, pos.line).markedSpans;
            if (spans) {
                for (let i = 0; i < spans.length; ++i) {
                    const span = spans[i];
    ...
    function forEachCodeMirror(f) {
        ...
        const byClass = document.getElementsByClassName("CodeMirror"), editors = [];
    ...
    function deleteNearSelection(cm, compute) {
        const ranges = cm.doc.sel.ranges, kill = [];
    ...
    function moveVisually(cm, line, start, dir) {
        ...
        const partPos = getBidiPartAt(bidi, start.ch, start.sticky), part = bidi[partPos];
    ...
    const commands = {
        ...
        delWrappedLineLeft: function (cm) {
            return deleteNearSelection(cm, function (range) {
                ...
                const leftPos = cm.coordsChar({left: 0, top: top}, "div");
        ...
        delWrappedLineRight: function (cm) {
            return deleteNearSelection(cm, function (range) {
                const top = cm.charCoords(range.head, "div").top + 5;
                const rightPos = cm.coordsChar({left: cm.display.lineDiv.offsetWidth + 100, top: top}, "div");
        ...
        goLineLeftSmart: function (cm) {
            return cm.extendSelectionsBy(function (range) {
                const top = cm.cursorCoords(range.head, "div").top + 5;
                const pos = cm.coordsChar({left: 0, top: top}, "div");
        ...
        insertSoftTab: function (cm) {
            const spaces = [], ranges = cm.listSelections(), tabSize = cm.options.tabSize;
            for (let i = 0; i < ranges.length; i++) {
        ...
        transposeChars: function (cm) {
            return runInOp(cm, function () {
                const ranges = cm.listSelections(), newSel = [];
                for (let i = 0; i < ranges.length; i++) {
                    if (!ranges[i].empty()) {
                        continue
                    }
    ...
    function onKeyPress(e) {
        ...
        const keyCode = e.keyCode, charCode = e.charCode;
    ...
    function onMouseDown(e) {
        const cm = this, display = cm.display;
        ...
        const pos = posFromMouse(cm, e), button = e_button(e), repeat = pos ? clickRepeat(pos, button) : "single";
        window.focus();
    ...
    function leftButtonStartDrag(cm, event, pos, behavior) {
        const display = cm.display;
        let moved = false;
        const dragEnd = operation(cm, function (e) {
            if (webkit) {
                display.scroller.draggable = false;
            }
    ...
    function leftButtonSelect(cm, event, start, behavior) {
        ...
        let ourRange, ourIndex, startSel = doc.sel;
        ...
        function extendTo(pos) {
            ...
            if (behavior.unit === "rectangle") {
                const ranges = [], tabSize = cm.options.tabSize;
                ...
                const left = Math.min(startCol, posCol), right = Math.max(startCol, posCol);
    ...
    function bidiSimplify(cm, range) {
        ...
        const index = getBidiPartAt(order, anchor.ch, anchor.sticky), part = order[index];
        ...
        const ch = from ? usePart.from : usePart.to, sticky = from ? "after" : "before";
    ...
    function registerEventHandlers(cm) {
        ...
        function farAway(touch, other) {
            ...
            const dx = other.left - touch.left, dy = other.top - touch.top;
    ...
    function indentLine(cm, n, how, aggressive) {
        ...
        const line = getLine(doc, n), curSpace = countColumn(line.text, null, tabSize);
        if (line.stateAfter) {
            line.stateAfter = null;
        }
    ...
    function hiddenTextarea() {
        const te = elt("textarea", null, null, "position: absolute; bottom: -1em; padding: 0; width: 1px; height: 1em; min-height: 1em; outline: none");
    ...
    function addEditorMethods(CodeMirror) {
        ...
        CodeMirror.prototype = {
            ...
            setOption: function (option, value) {
                const options = this.options, old = options[option];
            ...
            getTokenTypeAt: function (pos) {
                ...
                let before = 0, after = (styles.length - 1) / 2;
                const ch = pos.ch;
                let type;
                if (ch === 0) {
                    type = styles[2];
            ...
            getHelpers: function (pos, type) {
                ...
                const help = helpers[type], mode = this.getModeAt(pos);
            ...
            deleteH: methodOp(function (dir, unit) {
                const sel = this.doc.sel, doc = this.doc;
            ...
            findWordAt: function (pos) {
                const doc = this.doc, line = getLine(doc, pos.line).text;
    ...
    ContentEditableInput.prototype.init = function (display) {
        ...
        function onCopyCut(e) {
            ...
            const kludge = hiddenTextarea(), te = kludge.firstChild;
    ...
    ContentEditableInput.prototype.showPrimarySelection = function () {
        const sel = this.getSelection(), cm = this.cm, prim = cm.doc.sel.primary();
        const from = prim.from(), to = prim.to();
    ...
    ContentEditableInput.prototype.pollSelection = function () {
        ...
        const sel = this.getSelection(), cm = this.cm;
    ...
    ContentEditableInput.prototype.pollContent = function () {
        ...
        const cm = this.cm, display = cm.display, sel = cm.doc.sel.primary();
        ...
        const newTop = newText[0], oldTop = oldText[0], maxCutFront = Math.min(newTop.length, oldTop.length);
        while (cutFront < maxCutFront && newTop.charCodeAt(cutFront) === oldTop.charCodeAt(cutFront)) {
            ++cutFront;
        }
        const newBot = lst(newText), oldBot = lst(oldText);
    ...
    function locateNodeInLineView(lineView, node, offset) {
        ...
        const measure = lineView.measure, maps = measure.maps;
        ...
        for (; after; after = after.nextSibling) {
        ...
        let before = topNode.previousSibling, dist$1 = offset;
        for (; before; before = before.previousSibling) {
    ...
    TextareaInput.prototype.prepareSelection = function () {
        ...
        const cm = this.cm, display = cm.display, doc = cm.doc;
        const result = prepareSelection(cm);

        // Move the hidden textarea near the cursor to prevent scrolling artifacts
        if (cm.options.moveInputWithCursor) {
            const headPos = cursorCoords(cm, doc.sel.primary().head, "div");
            const wrapOff = display.wrapper.getBoundingClientRect(), lineOff = display.lineDiv.getBoundingClientRect();
    ...
    TextareaInput.prototype.showSelection = function (drawn) {
        const cm = this.cm, display = cm.display;
    ...
    TextareaInput.prototype.poll = function () {
        ...
        const cm = this.cm, input = this.textarea;
    ...
    TextareaInput.prototype.onContextMenu = function (e) {
        const input = this, cm = input.cm, display = cm.display, te = input.textarea;
        if (input.contextMenuPending) {
            input.contextMenuPending();
        }
        const pos = posFromMouse(cm, e), scrollPos = display.scroller.scrollTop;
        if (!pos || presto) {
            return
        } // Opera is difficult.
        ...
        const oldCSS = te.style.cssText, oldWrapperCSS = input.wrapper.style.cssText;
    ...
    function fromTextArea(textarea, options) {
        ...
        if (textarea.form) {
            ...
            if (!options.leaveSubmitMethodAlone) {
                ...
                try {
                    const wrappedSubmit = form.submit = function () {
                        save();
