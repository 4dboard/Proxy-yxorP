...
(function (mod) {
    ...
    function findMatchingClose(iter, tag) {
        ...
        for (; ;) {
            ...
            let end;
            const startLine = iter.line, startCh = iter.ch - (next ? next[0].length : 0);
    ...
    function findMatchingOpen(iter, tag) {
        ...
        for (; ;) {
            ...
            const endLine = iter.line, endCh = iter.ch;
    ...
    CodeMirror.findMatchingTag = function (cm, pos, range) {
        ...
        const end = toTagEnd(iter), to = end && Pos(iter.line, iter.ch);
        const start = end && toTagStart(iter);
        if (!end || !start || cmp(iter, pos) > 0) return;
        const here = {from: Pos(iter.line, iter.ch), to: to, tag: start[2]};
    ...
    CodeMirror.scanForClosingTag = function (cm, pos, name, end) {
        const iter = new Iter(cm, pos.line, pos.ch, end ? {from: 0, to: end} : null);
