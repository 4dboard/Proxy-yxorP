...
(function (mod) {
    ...
    CodeMirror.showHint = function (cm, getHints, options) {
        ...
        if (options) for (let prop in options) newOpts[prop] = options[prop];
    ...
    Completion.prototype = {
        ...
        cursorActivity: function () {
            ...
            const pos = this.cm.getCursor(), line = this.cm.getLine(pos.line);
        ...
        update: function (first) {
            ...
            const self = this, myTick = ++this.tick;
    ...
    function Widget(completion, data) {
        ...
        for (let i = 0; i < completions.length; ++i) {
            const elt = hints.appendChild(ownerDocument.createElement("li")), cur = completions[i];
        ...
        cm.on("scroll", this.onScroll = function () {
            const curScroll = cm.getScrollInfo(), editor = cm.getWrapperElement().getBoundingClientRect();
    ...
    CodeMirror.registerHelper("hint", "fromList", function (cm, options) {
        ...
        let term, from = CodeMirror.Pos(cur.line, token.start);
        const to = cur;
