<!DOCTYPE html>
<html lang="en" class="<?= $this->helper('theme')->pageClass() ?>" data-base="<?= $this->base('/') ?>"
      data-route="<?= $this->route('/') ?>" data-version="<?= $this->retrieve('app.version') ?>"
    ...
    <aside class="app-container-aside">
        ...
        <div class="app-container-aside-float">
            ...
            <kiss-navlist space="small">
                <ul>
                    <li>
                        <a class="kiss-flex kiss-flex-center" href="<?= $this->route('/system/users/user') ?>"
                           aria-label="<?= t('Account') ?>" kiss-tooltip="right">
                            <icon>account_circle</icon>
                        </a>
                    </li>
                    <li>
                        <a class="kiss-flex kiss-flex-center" href="<?= $this->route('/system') ?>"
                           aria-label="<?= t('Settings') ?>" kiss-tooltip="right">
                            <icon>tune</icon>
                        </a>
                    </li>
                    <?php if ($this->helper('acl')->isAllowed('app.users.manage')): ?>
                        <li>
                            <a class="kiss-flex kiss-flex-center" href="<?= $this->route('/system/users') ?>"
                               aria-label="<?= t('Users') ?>" kiss-tooltip="right">
        ...
        <app-header>
            <kiss-container class="kiss-flex kiss-flex-middle">
                ...
                <?php if ($this->path('#app:') != $this->path('#root:')): ?>
                    <div class="kiss-margin-small-left"><span
                            class="kiss-badge kiss-color-primary kiss-badge-outline kiss-text-upper"><?= str_replace(['-', '_'], ' ', basename($this->path('#root:'))) ?></span>
                    </div>
                <?php endif ?>
        ...
        <kiss-navlist>
            <ul>
                ...
                <li><a class="kiss-flex kiss-flex-middle" href="<?= $this->route('/system/users/user') ?>">
                        <icon class="kiss-margin-small-right">account_circle</icon> <?= t('Account') ?></a></li>
                <li class="kiss-nav-divider"></li>
            ...
            <div class="kiss-padding">
                <kiss-navlist>
                    <ul>
                        <li class="<?= ($this->request->route == '/') ? 'active' : '' ?>">
                            <a href="<?= $this->route('/') ?>">
                                <kiss-svg class="kiss-margin-small-right" src="<?= $this->base('app:icon.svg') ?>"
                                          width="25" height="25"></kiss-svg>
                                <?= t('Dashboard') ?>
                            </a>
                        </li>
                        <?php foreach ($this->helper('menus')->menu('modules', true) as $group => $links): ?>
