...
(function (mod) {
    ...
    function ensureDeps(mode, cont, options) {
        const modeObj = CodeMirror.modes[mode], deps = modeObj && modeObj.dependencies;
        if (!deps) return cont();
        const missing = [];
    ...
    CodeMirror.requireMode = function (mode, cont, options) {
        ...
        const file = options && options.path ? options.path(mode) : CodeMirror.modeURL.replace(/%N/g, mode);
