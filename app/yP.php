...
class yP
{
    ...
    public static function autoLoader(string $root): void
    {
         ...
         */
        foreach (glob("$root/*") as $path) if (is_dir($path)) self::autoLoader($path); else if ((str_contains($path, 'Interface') || str_contains($path, 'Interface') )&& str_contains($path, EXT_PHP)) require_once $path; else $classes[] = $path;
        /**
