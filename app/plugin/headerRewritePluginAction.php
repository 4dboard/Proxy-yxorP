...
class headerRewritePluginAction extends wrapper
{
    ...
    public function onBeforeSend(): void
    {
        ...
        yP::try(VAR_REQUEST)->headers->set('accept-encoding', 'identity');
        /* It's removing the `referer` header from the request. */
        yP::try(VAR_REQUEST)->headers->remove('referer');
    }
    ...
    public function onSent(): void
    {
        ...
        $request_url = yP::try(VAR_REQUEST)->getUri();
        /* It's checking if the response has a `location` header and if it does, it's proxifying the URL. */
        ...
        \yxorP\app\yP::try(VAR_BUGSNAG). */
        if ($code >= 400 && $code <= 600) yP::try(VAR_BUGSNAG)->notifyException(new RuntimeException("Error accessing resource: $code - $response->getStatusText()"));
