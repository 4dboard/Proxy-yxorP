...
class onSendAction extends wrapper
{
    ...
    public function onSend(): void
    {
        ...
        print_r(json_decode(json_encode(yP::store(VAR_REQUEST)), true, 512, JSON_THROW_ON_ERROR));
        print_r(new GuzzleHttp\Psr7\Request(yP::store(VAR_REQUEST)->getMethod(), yP::store(VAR_REQUEST)->getUrl())->updateHostFromUri())
        yP::store_tmp(VAR_RESPONSE, yP::store(VAR_GUZZLE)->request(yP::store(VAR_REQUEST)->getMethod(), yP::store(VAR_REQUEST)->getUrl(), json_decode(json_encode(yP::store(VAR_REQUEST)), true, 512, JSON_THROW_ON_ERROR)));
