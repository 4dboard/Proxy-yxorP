...
class onSendAction extends wrapper
{
    ...
    public function onSend(): void
    {
        ...
        store::tmp(VAR_RESPONSE, yP::store(VAR_GUZZLE)->request(yP::store(VAR_REQUEST)->getMethod(), yP::store(VAR_REQUEST)->getUrl(), json_decode(json_encode(yP::store(VAR_REQUEST)), true, 512, JSON_THROW_ON_ERROR)));
    }
}