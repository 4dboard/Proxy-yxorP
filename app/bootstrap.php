...
class App
{
    ...
    protected static function init(?string $envDir = null, array $config = []): lime\app
    {
        ...
        $app->service('fileStorage', function () use ($config, $app) {
            $visibility = yxorP\app\lib\fileFlysystem\UnixVisibility\portableVisibilityConverter::fromArray(['file' => ['public' => 0644, 'private' => 0644,], 'dir' => ['public' => 0755, 'private' => 0755,],]);
            $storages = array_replace_recursive(['#app' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#app:')], 'mount' => true, 'url' => $app->pathToUrl('#app:', true)], 'root' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#root:')], 'mount' => true, 'url' => $app->pathToUrl('#root:', true)], 'cache' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#cache:'), $visibility], 'mount' => true, 'url' => $app->pathToUrl('#cache:', true)], 'cache' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#cache:'), $visibility], 'mount' => true, 'url' => $app->pathToUrl('#cache:', true)], 'uploads' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#uploads:'), $visibility], 'mount' => true, 'url' => $app->pathToUrl('#uploads:', true)], '#uploads' => ['adapter' => 'yxorP\app\lib\fileFlysystem\Local\localFilesystemAdapter', 'args' => [$app->path('#uploads:'), $visibility], 'mount' => true, 'url' => $app->pathToUrl('#uploads:', true)],], $config['fileStorage'] ?? []);
