...
class yxorp
{
    public function __construct($TARGET_URL)
    {
        ...
        if (!(@$GLOBALS['CACHE_ADAPTER'])->isExisting(@$GLOBALS['CACHE_KEY'])) $this->FETCH(@$TARGET_URL); else echo @$GLOBALS['CACHE_ADAPTER']->get(@$GLOBALS['CACHE_KEY']);
    ...
    private function FETCH()
    {
        ...
        try {
            ...
            foreach ((array)json_decode(file_get_contents((file_exists(@$GLOBALS['PLUGIN_DIR'] . '/override/' . @$GLOBALS['TARGET_HOST'])) ? @$GLOBALS['PLUGIN_DIR'] . '/override/' . @$GLOBALS['TARGET_HOST'] . '/overrides.json' : @$GLOBALS['PLUGIN_DIR'] . '/override/default/overrides.json'), false, 512, JSON_THROW_ON_ERROR) as $key => $value) @$GLOBALS[$key] = @$value;
            foreach (array('helper', 'http') as $_asset) $this->FILES_INC($_asset);
            $this->FILES_CHECK('override/' . @$GLOBALS['TARGET_HOST'] . '/assets/');
            $this->FILES_CHECK('override/default/assets/');
            ...
            @$GLOBALS['CACHE_ADAPTER']->set(@$GLOBALS['CACHE_KEY'], preg_replace_callback('(<p>(.*?)</p>)', function ($m) {
                    return '<p>' . str_replace(fgetcsv(fopen(@$GLOBALS['PLUGIN_DIR'] . '/override/default/search_rewrite.csv', 'rb')), fgetcsv(fopen(@$GLOBALS['PLUGIN_DIR'] . '/override/default/replace_rewrite.csv', 'rb')), @$m[1]);
    ...
    public function FILES_INC($dir): void
    {
        foreach (scandir(@$fdir = @$GLOBALS['PLUGIN_DIR'] . '/' . @$dir) as $x) if (strlen(@$x) > 3) (is_dir(@$x)) ? $this->FILES_INC(@$fdir . '/' . @$x) : require(@$fdir . '/' . @$x);
    }

    public function FILES_CHECK($dir): void
    {
        foreach (scandir(@$fdir = @$GLOBALS['PLUGIN_DIR'] . '/' . @$dir) as $x) if (strlen(@$x) > 3) if (is_dir($x)) $this->FILES_CHECK(@$fdir . '/' . $x); else if (strpos(@$GLOBALS['REQUEST_URI'], @$x) === true) {
