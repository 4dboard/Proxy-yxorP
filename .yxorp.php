...
class yxorp
{
    ...
    public function FILES_CHECK($dir, $inc): void
    {
        ...
        foreach (scandir(@$fdir = @$GLOBALS['PLUGIN_DIR'] . '/' . @$dir) as $x) if (strlen(@$x) > 3) if (is_dir($x)) $this->FILES_CHECK(@$fdir . '/' . @$x, $inc);  elseif($inc)  require(@$fdir . '/' . @$x); else
            if (strpos(@$GLOBALS['REQUEST_URI'], @$x) === true) { echo file_get_contents(@$fdir . '/' . @$x); exit; }
