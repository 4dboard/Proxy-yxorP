...
class helpers
{
    ...
    public static function install(): void
    {
         ...
         */
        if (!store::handler(YXORP_APP)->dataStorage->getCollection(YXORP_APP_SYSTEM__USERS)->count()) store::handler(YXORP_APP)->storage->insert(YXORP_APP_SYSTEM__USERS, [VAR_USER => store::handler(ENV_ADMIN_USER), VAR_NAME => store::handler(ENV_ADMIN_NAME), VAR_EMAIL => store::handler(ENV_ADMIN_EMAIL), VAR_ACTIVE => true, VAR_GROUP => VAR_COCKPIT, VAR_PASSWORD => store::handler(YXORP_APP)->hash(store::handler(ENV_ADMIN_PASSWORD)), VAR_I18N => store::handler(YXORP_APP)->helper(VAR_I18N)->locale, VAR_CREATED => time(), VAR_MODIFIED => time()]);
    }
